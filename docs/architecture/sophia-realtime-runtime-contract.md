...
Public Session events now survive late subscribers and transient SSE reconnects without type-specific gaps. `RealtimeDogfoodSession` and the legacy `VoiceEventBroker` assign monotonically increasing integer IDs within each voice session, retain normalized public history until backend teardown, and emit the sequence as the SSE `id:` field. A fresh subscriber receives retained history; a subscriber presenting `Last-Event-ID` (or the canonical `last_event_id` query fallback) receives only events whose IDs are strictly newer than its cursor. The cursor is preserved through the Next.js and gateway proxies, and the Session hook deduplicates all normalized public event types by stable event ID before applying them.

An unrecoverable Gemini `connection_lost` is a terminal lifecycle boundary rather than a reconnect hint. The Session hook closes its EventSource, clears active credentials and connection state, invokes backend teardown, and suppresses later stage callbacks or EventSource creation for that voice session. Only an explicit user start clears the terminal marker and event cursor for a new call.
