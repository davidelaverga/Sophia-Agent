...
- **L1 — input validity:** espeak opened 1 of 4 first turns and 0 of 2 under real-time delivery. On the Starter 0.5 CPU / 512 MB worker, the browser audio graph ran at 61% of real time with gaps of at least 300 ms. J6's French partial and missing second sentence cannot be attributed to the product from this input. J6's timing archive is a historical fallback: its rows were purged before browser timestamp provenance could be compared. R1/R3 each retained 71 browser-stamped frame rows whose stored timestamps matched exactly, and their timely delivery did not produce a reply. PR #156 proposes natural pinned inputs, measured active-run worker admission, served-frontend adapter preflight, and browser-time invalid-test classification; it is not deployed.
- **L2 — R2:** the effective Production frontend control adapter had not been reopened (C1), so R2 never supplied provider evidence.
- **L3 — empty-session End:** R1's supported End succeeded. R3's End returned 503 `voice_lab_canonical_transcript_invalid` after a zero-turn canonical commit; automatic recovery later proved `live_resources_zero`. That recovery is not a successful supported End. R3 was an ad hoc Lab run with `scenario_id=null` and `scenario_version=null`, while R1 had both fields. The End claim permits absent scenario identity, but the deployed canonical transcript validator required nonempty strings. A local R3-shaped End handler request reproduced the post-commit 503 with exactly those two invalid fields; PR #157 makes them required but nullable and returns 202 for the same request. The original request body and pre-End state were not retained as a before image. This repair is local and not deployed; supported production End remains unverified after the patch.
- **L4 — resampler:** the microphone downsampler was unfiltered during this window. PR #155 independently proposes stateful filtered conversion with 44.1 kHz fractional-phase correction and low-rate capture support. No controlled comparison has tied it to J6; it is not deployed.
