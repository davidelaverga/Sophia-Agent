...
- R3 remeasurement proved direct non-inferred storage, two distinct rows without merge, initial metadata, stable returned IDs, exact operation-marker reconciliation, repeated reconciliation convergence, page-size-one pagination, subject isolation, namespace metadata, and semantic search. The current API key denied both exact-ID and batch v1 deletion with HTTP 403; authenticated-dashboard cleanup removed all three probe rows and paginated API enumeration proved both synthetic subjects at zero. The user has now authorized one same-project replacement key only if the action-time exact-delete probe still fails, replacement of only the existing Gateway `MEM0_API_KEY`, retention of the old provider key for rollback, and a full terminal-zero R3 rerun. Projection remains closed until that proof passes.
