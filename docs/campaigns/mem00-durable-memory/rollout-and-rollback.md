...
2. R1 local containment: implemented in part; current per-model retained-history and complete consumer-matrix gaps remain promotion blockers. Deterministic, architecture, redaction, identity, consumer, and fault tests must remain green.
...
4. R3 synthetic Mem0 contract proof: refreshed and passing on c41bbb102022c82a50c8afe4080068e54ea0c409 at2026-09-07T19:48:47Z. The previously approved same-project replacement credential resolves the earlier403; do not repeat credential/configuration changes. Current three-fixture add/get/search/metadata/reconcile/paginate/isolation/delete proof and both terminal-zero subjects are in `repair-deployment-c41bbb1.json`. Remeasure hosted behavior on every certification run; this probe is not a complete product canary.
5. R4 dark production: previously approved additive schema and compatible deployment are applied. Current contract is enforced mem00.v1/epoch1; do not mistake the historical all-flags-off/approval-pending notes below for current state.
6. R5 synthetic cohort: active only for the named certification principal, but incomplete. Run MEM-P01 through MEM-P08 and join product rows, provider bindings, logs/metrics, LangSmith, and UI. Cross-candidate partial journeys cannot satisfy the five-canary gate.
