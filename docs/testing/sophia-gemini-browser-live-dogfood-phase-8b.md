...
- Prompt/instructions: Gemini setup now uses `voice/realtime/sophia_prompt.py`, which assembles from the same canonical Sophia sources used by the companion path: `skills/public/sophia/soul.md`, `voice.md`, `techniques.md`, `coordination_core.md`, `companion_delegation.md`, platform guidance from `PlatformContextMiddleware`, optional context/ritual files, and the voice artifact contract from `ArtifactMiddleware`.
