...
When this skill is used inside Sophia Builder, treat it as research/review input
for a larger deliverable such as `pdf-report`. Save any intermediate review
under `/mnt/user-data/outputs/`, but do **not** call `present_files` and do not
treat the review as the final artifact unless the user explicitly asked only
for a standalone paper review. Final delivery still goes through
...
- In Sophia Builder, do not call `present_files`; use `emit_builder_artifact`
  only when the review itself is the requested final deliverable.
