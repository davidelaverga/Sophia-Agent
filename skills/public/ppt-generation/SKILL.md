...
2. For each slide provide:
   ...
   - repair_anchor_ids: exactly two distinct short HTML ids naming the strict repair witness pair on the slide; each declared id must exactly match its anchor's `id`
   ...
   - both independent repair-addressable layout anchors declared by `repair_anchor_ids`: non-nested `section` or `div` direct children of the service-owned `main`, each with visible semantic text and an HTML `id` unique within its slide; other layout elements remain allowed. The same two short anchor IDs may be reused in separate slide fragments so shared `#id` rules scale. Each anchor `id` must match `[a-z][a-z0-9_-]{0,31}`—a lowercase ASCII letter followed by at most 31 lowercase ASCII letters, digits, `_`, or `-`, for a maximum of 32 characters. Each anchor's `data-deck-id` must be unique within its slide, its `data-deck-role` must be nonempty, and it must use `data-deck-required="true"`. Canonical example: `repair_anchor_ids=["hero","proof"]`, then start `html_body` with `<section id="hero" data-deck-id="hero" data-deck-role="title" data-deck-required="true">...</section><div id="proof" data-deck-id="proof" data-deck-role="narrative" data-deck-required="true">...</div>`

3. Set `authoring_contract="compact_model_html_v2"`. Provide one concise `deck_stylesheet` containing all fresh-deck CSS. Reuse shared classes and target each `html_body` under 4 KiB. The combined `html_body` bytes must stay within 4 KiB times the slide count; slides may borrow unused body budget, with each slide capped at the hard 6 KiB ceiling. Omit `slide_css` or pass an empty string for every slide so its full 1 KiB channel remains available to a later authenticated repair overlay, and keep the complete tool arguments under 48 KiB. The stylesheet must give the 1920x1080 canvas an explicit opaque background. In `deck_stylesheet`, give each repair anchor's short `#id` a flat rule with `position:absolute`, `box-sizing:border-box`, `margin:0`, and finite-pixel `left`, `top`, `width`, and `height`; keep its effective baseline box at least 48x24px and wholly on canvas, with at least 8px of free canvas on one horizontal or vertical side so the whole anchor can translate by 8px. Use real content containers as the anchors; never duplicate visible content into extra positioned overlay anchors merely to satisfy the contract. No selector or inline style matching an anchor may declare auto, nonzero, or otherwise non-literal-zero physical margin, or any logical or vendor margin property; remove an unsafe declaration rather than overriding it with a later `margin:0` reset. Grouped physical `margin:0` is safe but unnecessary; reset margins on anchor descendants with separate descendant selectors. Keep required anchor geometry out of `slide_css` and inline styles. Do not use `!important`, `right`, `bottom`, `inset`, min/max sizing, `transform`, or `opacity` on an anchor, and do not use at-rules or nested CSS. Flex and grid may be used inside the anchors, so composition and visual direction remain subject-derived. In canvas-global coordinates, keep every pair of unrelated visible text-bearing rectangles disjoint with at least a 16px gutter. Intentional containment is allowed only inside a non-text background with no native text frame; exact-edge connector or background touching remains allowed. Keep non-bleed geometry inside that canvas. Absolutely positioned descendants inside a positioned parent use parent-local `left`/`top`; never repeat the parent's slide-global offset on a nested child.
