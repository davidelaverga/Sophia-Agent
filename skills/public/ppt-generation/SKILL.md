...
3. Set `authoring_contract="compact_model_html_v2"`. Provide one concise `deck_stylesheet` containing shared model-authored CSS. Reuse shared classes, keep each `html_body` under 3 KiB, keep `slide_css` under 1 KiB and exceptional, and keep the complete tool arguments under 48 KiB. The stylesheet must give the 1920x1080 canvas an explicit opaque background.
