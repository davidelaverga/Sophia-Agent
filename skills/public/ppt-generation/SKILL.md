...
Every slide is a single 16:9 image with all visible title, caption, labels,
diagrams, and layout baked into the bitmap. The PPTX compiler places that image
...
1. Plan the deck: one line per slide with `title`, core idea, treatment,
   diagram family, and caption/takeaway.
...
- top 14% title band.
- Center 75% visual safe area.
- bottom 11% caption/takeaway band for content, architecture, process,
...
```text
A professional presentation slide, 16:9.
Reserve the top 14% for title, bottom 11% for one-line caption/takeaway, and center 75% for the visual.
Title band text: "{title}".
Bottom caption band text: "{caption}".
...
- Title: 4-9 words.
- Caption: one sentence, ideally 8-16 words.
...
replace the bottom caption/takeaway.
...
- Baked caption/takeaway visible on content slides.
- No overlap between title, center visual, and bottom caption.
