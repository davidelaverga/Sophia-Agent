...
- Every slide needs at least two independent repair-addressable layout anchors: non-nested `section` or `div` direct children of the service-owned `main`, each containing visible semantic text. Each anchor needs an HTML `id` unique within its slide; the same two short anchor IDs may be reused in separate slide fragments so shared `#id` rules scale. Each anchor `id` must match `[a-z][a-z0-9_-]{0,31}`: a lowercase ASCII letter followed by at most 31 lowercase ASCII letters, digits, `_`, or `-`, for a maximum of 32 characters. Each anchor's `data-deck-id` must be unique within its slide, its `data-deck-role` must be nonempty, and it must use `data-deck-required="true"`. In `deck_stylesheet`, target each short `#id` with a flat rule that explicitly sets `position:absolute`, `box-sizing:border-box`, `margin:0`, and finite-pixel `left`, `top`, `width`, and `height`; keep each box at least 48x24px and wholly on the canvas. No other CSS selector matching an anchor may declare a nonzero margin or any logical or vendor margin property; reset margins on anchor descendants with separate descendant selectors. Keep this anchor geometry out of `slide_css` and inline styles. Do not use `!important`, `right`, `bottom`, `inset`, min/max sizing, `transform`, or `opacity` on an anchor, and do not use at-rules or nested CSS. Flex and grid remain available inside either anchor.
