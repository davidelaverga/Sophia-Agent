...
Fresh deck builds are DeckBuildService builds. The builder submits slide intent
through `prepare_deck_build`; the harness owns deck compilation. Later artifact
edits will target manifest components, but fresh deck creation does not expose
lower-level deck tools to the model.
