...
3. Use `generate_visual_asset` for numeric charts and data-shaped visuals, or
   `generate_excalidraw_diagram` for technical diagrams. Both write local
   SVG/PNG assets under `/mnt/user-data/outputs/visuals/`.
...
concept, comparison, cycle, and sequence diagrams. Use the image-generation
skill only for illustrative content: hero images, section covers, conceptual
scenes, or polished atmosphere. PPTX uses a presentation-specific cap; the cap
never applies to charts or Excalidraw diagrams. If image generation fails,
