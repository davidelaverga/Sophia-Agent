...
## Sophia Builder Adapter

When this skill is used inside Sophia Builder, treat it as research synthesis
input for `pdf-report` unless the user explicitly asked for a standalone SLR
Markdown artifact. Do **not** call `present_files`; final delivery still goes
through `emit_builder_artifact`. If the `task` subagent tool is not available in
the builder runtime, cap the review at 8-10 papers and synthesize in the current
run instead of looping or failing the whole PDF report.
...
When the `task` tool is available, delegate extraction to subagents via the
`task` tool instead of extracting metadata yourself. If `task` is unavailable
inside Sophia Builder, cap the set at 8-10 papers and perform a compact
in-context extraction rather than failing a larger PDF report. Specifically,
avoid the following unless the fallback cap is active:
...
Instead, prefer the `task` tool to spawn subagents. The reason: extracting
10-50 papers in your own context consumes too many tokens and degrades
synthesis quality in Phase 4. Each subagent runs in an isolated context with
only its batch of papers, producing cleaner extractions.
...
Save the full report to `/mnt/user-data/outputs/slr-<topic-slug>-<YYYYMMDD>.md` where `<topic-slug>` is a lowercased hyphenated version of the topic (e.g. `transformer-attention`). In upstream DeerFlow you would call `present_files`; in Sophia Builder, do not call `present_files`. Use the saved report as input to `pdf-report`, or call `emit_builder_artifact` only when the SLR Markdown is the final requested artifact.
...
5. Phase 5: save to `slr-transformer-attention-20260409.md`; in Sophia Builder, feed it into `pdf-report` or emit it only if Markdown SLR was the final requested artifact.
...
- **Full workflow prerequisite: `subagent_enabled` must be `true`**. Phase 3 uses the `task` tool for parallel metadata extraction. This tool is only loaded when `subagent_enabled` is set to `true` in the runtime config (`config.configurable.subagent_enabled`). Without it, cap the Sophia Builder workflow at 8-10 papers and synthesize compactly in-context.
...
- **Phase 3 prefers subagents**. If subagents are unavailable, do not claim to execute the Phase 3 parallel plan; use the Sophia Builder cap above, or narrow/split the request into a smaller manual review.
