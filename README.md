...
Build/deliverable requests ("make me a report about X") are transient task history, not durable facts — they are skip-listed at extraction and never stored, and the Builder's memory retrieval is restricted to `preference` so a prior task's subject can't contaminate a new build. See `backend/CLAUDE.md` → "Builder memory-contamination guard".
...
Telegram chats also trigger the Sophia offline pipeline after 10 minutes of inactivity: handoffs, Mem0 candidates, and a sparse recap envelope are written, then the bot DMs the user a `LoginUrl` button into `/recap/{session_id}` for memory review. Requires `/setdomain` configured in BotFather against the host of `SOPHIA_WEB_BASE_URL`.
