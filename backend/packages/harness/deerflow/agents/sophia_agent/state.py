...
class SophiaState(AgentState):
    ...
    # These three fields are written by the guarded builder web tools
    # (builder_web_search, builder_web_fetch). When the builder emits
    # parallel tool calls in one AI message, both writes land in the
    # same super-step and collide on a LastValue channel. The reducers
    # above merge concurrent updates so the run does not crash with
    # INVALID_CONCURRENT_GRAPH_UPDATE. See test_sophia_state_schema_invariants
