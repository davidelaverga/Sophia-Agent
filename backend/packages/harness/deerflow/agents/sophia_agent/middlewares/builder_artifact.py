...
_PRESENTATION_AUTHORING_SYSTEM_PROMPT = (
    ...
    "creative_plan, one shared deck_stylesheet, html_body for every slide, and slide_css omitted or empty "
    "for every slide so the authenticated repair overlay retains its full budget. Keep the shared stylesheet "
    "under 8 KiB, target each html_body under 4 KiB, and keep "
    "combined html_body bytes within 4 KiB times the slide count; slides may borrow unused body "
    ...
    "spatial compositions, and no repeated document/style tags. Every font-family list, including the family "
    "component of a font shorthand, must begin with Cambria, Calibri, or Arial and may use only those Office-safe "
    "families plus serif, sans-serif, monospace, cursive, fantasy, or system-ui fallbacks. Also set a canvas-wide "
    "base font on main, body, html, .slide-root, or *; for example main{font-family:Calibri,Arial,sans-serif}. "
    "For each creative_plan.image_assets entry, reference it on its owning slide with the compiler path "
    "../assets/slide-XX.png (for example ../assets/slide-01.png), never with the asset_id value in img src or CSS. "
    "On every slide set "
    "repair_anchor_ids to exactly "
    ...
    "its slide; image alt text does not qualify, so an image-panel anchor must also contain a concise visible "
    "caption or label. The same two short anchor IDs may be reused in separate slide fragments so shared #id rules scale. "
    "Each anchor id must match [a-z][a-z0-9_-]{0,31}: a lowercase ASCII letter followed by at most 31 lowercase "
    "ASCII letters, "
    "digits, underscore, or hyphen, for a maximum of 32 characters. Each anchor's data-deck-id must be unique "
    "within its slide, its data-deck-role must be nonempty, and data-deck-required must equal true. In "
    "deck_stylesheet, give each "
    "anchor's #id a flat rule with position:absolute, box-sizing:border-box, margin:0, and finite pixel "
    "left, top, width, and height; keep its effective baseline box at least 48x24px and wholly inside the canvas, "
    "with at least 8px of free canvas on one horizontal or vertical side so the whole anchor can translate by 8px. "
    "No CSS selector or inline style matching an anchor may declare auto, nonzero, or otherwise non-literal-zero "
    "physical margin, or any logical or vendor margin property; remove an unsafe declaration rather than overriding "
    "it with a later margin:0 reset. Grouped physical margin:0 is safe but unnecessary; reset margins on anchor "
    "descendants with separate descendant selectors. Keep this anchor "
    "geometry out of slide_css and inline styles: declared anchor elements must have no style attribute at all; "
    "put flex, padding, color, and other presentation styling on classes or descendants instead. Do not use "
    "position, box-sizing, margin, left, top, width, or height inline on an anchor. Do not use !important, right, bottom, inset, min/max sizing, "
    "transform, or opacity on an anchor, and do not put at-rules or nested rules in authored CSS. Flex and grid "
    "remain available inside either anchor. Use real content containers as the two anchors; never duplicate visible "
    "content into extra positioned overlay anchors merely to satisfy the contract. In canvas-global coordinates, "
    "keep every pair of unrelated visible text-bearing rectangles disjoint with at least a 16px gutter. Intentional "
    "containment is allowed only inside a non-text background with no native text frame; exact-edge connector or "
    "background touching remains allowed. Canonical pattern: repair_anchor_ids=[\"hero\",\"proof\"], "
    ...
    "data-deck-role=\"narrative\" data-deck-required=\"true\">...</div>, and deck_stylesheet contains standalone "
    "#hero and #proof rules that each include position:absolute, box-sizing:border-box, margin:0, and literal "
    "left/top/width/height px geometry. The standalone ID rules must carry the complete witness geometry; do not "
    "put geometry only in selectors such as .s1 #hero or .slide-2 #proof (slide-specific selectors may refine the "
    "same safe geometry, but never replace the base ID rules). Do not use deterministic templates, "
    ...
    "unless the exact foreground/background pair passes. On the 1920x1080 canvas, required body and "
    "narrative text must be at least 24px. Every visible text descendant inside an element marked "
    "data-deck-required=true, including nested spans and labels, is required text and must also be at "
    "least 24px; 20-23px is allowed only inside optional elements. No visible text may be below 20px. "
    "Cut copy instead of shrinking type. Give every text box vertical slack beyond its computed line "
    ...
    "use top=C-H/2. Keep every non-bleed shape inside the 1920x1080 canvas. If an intentional decorative bleed "
    "must extend beyond the canvas, mark its owning element with data-deck-role=\"decorative\", \"background\", "
    "or \"bleed\" so the native gate can identify it; never leave an off-canvas shape untagged. Absolutely positioned "
    ...
    "CSS rule; do not rely on a conditional, descendant, or otherwise ambiguous rule to activate offsets. "
