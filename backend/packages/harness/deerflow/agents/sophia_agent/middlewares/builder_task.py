...
class BuilderTaskMiddleware(AgentMiddleware[BuilderTaskState]):
    ...
    def before_agent(self, state: BuilderTaskState, runtime: Runtime) -> dict | None:
        ...
        pptx_visual_guidance = (
            ...
            "baked in — those are real HTML text): draft the slide plan, then write ONE JSON manifest listing "
            "every slide image including the cover/hero and call "
            "`image-generation/scripts/generate.py --manifest <path>` ONCE. Use consistent shared style "
            "instructions across all prompt JSON files. If the manifest is rejected "
