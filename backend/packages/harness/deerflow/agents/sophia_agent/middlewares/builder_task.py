...
class BuilderTaskMiddleware(AgentMiddleware[BuilderTaskState]):
    ...
    def before_agent(self, state: BuilderTaskState, runtime: Runtime) -> dict | None:
        ...
        sections.append(
            ...
            "- For text deliverables (markdown, html, plain text, code) under ~4,000 words: write the complete file in a single write_file_tool call. Do NOT split the same file across multiple write_file_tool calls — if output risks exceeding the write budget, ship a tighter draft instead of fragmenting.\n"
            "- For text deliverables OVER ~4,000 words (long research reports, book-length outputs): "
            "prefer a generator script approach even for markdown — write a short Python script "
            "that assembles the full document from typed sections and runs in under 90 seconds. "
            "A single synchronous write_file_tool call for a very long text can blow through the "
            "per-iteration soft cap (90s) and push the task past its overall timeout before "
            "emit_builder_artifact has a chance to fire.\n"
