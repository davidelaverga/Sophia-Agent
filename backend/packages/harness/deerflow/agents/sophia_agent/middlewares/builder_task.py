...
class BuilderTaskMiddleware(AgentMiddleware[BuilderTaskState]):
    ...
    def before_agent(self, state: BuilderTaskState, runtime: Runtime) -> dict | None:
        ...
        sections.append(
            ...
            "- For text deliverables (markdown, html, plain text, code) under ~2,500 words: write the complete file in a single write_file_tool call. Do NOT split the same file across multiple write_file_tool calls — if output risks exceeding the write budget, ship a tighter draft instead of fragmenting.\n"
            "- For text deliverables OVER ~2,500 words (long research reports, book-length outputs): "
