...
def _render_active_block(task: dict) -> str:
    ...
    return (
        "<build_status>\n"
        f"A builder task is in flight (running for {runtime_min} min, type: {summary}). "
        "Don't relaunch — the wrapper rejects duplicates and you cannot start a second build "
        "until this one finishes. If the user asks how it's going, briefly acknowledge "
        "progress and stay present. If the user did NOT ask, do NOT bring this up unprompted "
        "unless directly relevant to what they just said.\n"
        "</build_status>"
    )
...
def _render_terminal_block(task: dict) -> str:
    ...
    return (
        "<build_status>\n"
        f"The builder just finished a task ({minutes_ago} min ago). The artifact has been "
        "delivered separately to the user (Telegram chat / web view). If the user asks about "
        "it, reference the delivered artifact naturally — don't recite a file path or claim "
        "the file is unreachable. Don't bring this up unprompted unless directly relevant.\n"
        "</build_status>"
    )
