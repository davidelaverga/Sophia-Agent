...
def _render_terminal_block(task: dict) -> str:
    ...
    return (
        ...
        "calls).\n"
        f"    2. Call start_builder_task(description=<brief that references the prior "
        f"{task_type} artifact inline>, task_type=\"{task_type}\"). Reference the previous "
        "artifact's name/topic in the description so the builder picks it up as a v2.\n"
        "    3. emit_artifact with takeaway like \"Got it — kicking off a fresh build that "
