...
def _render_active_block(task: dict) -> str:
    ...
    return (
        ...
        "    3. End the turn.\n"
        "\n"
        "- Stop cues — \"stop\", \"cancel\", \"nevermind\", \"abort\", \"don't bother\":\n"
...
def _render_terminal_block(task: dict) -> str:
    ...
    return (
        ...
        "The artifact has been delivered separately to the user (Telegram chat / web view). "
        "If the user asks about it, reference the delivered artifact naturally — don't recite "
        "a file path or claim the file is unreachable.\n"
        "\n"
        "If the user references a previous build by description (\"that doc we started\", \"the "
        "research from earlier\", \"all my builds\"), call list_async_tasks() to recall recent "
        "task_ids and ack with a short summary of what's listed. If they then ask for details "
        "on a specific build, call check_async_task(task_id) on the NEXT turn — one lifecycle "
        "tool per turn, never chain two on the same turn. emit_artifact once per turn carries "
        "the ack.\n"
        "\n"
