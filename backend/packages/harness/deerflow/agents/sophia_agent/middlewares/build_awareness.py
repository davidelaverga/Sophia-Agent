...
def _render_active_block(task: dict) -> str:
    ...
    return (
        ...
        "    1. Call list_async_tasks(status_filter=\"running\").\n"
...
def _render_terminal_block(task: dict) -> str:
    ...
    return (
        ...
        "research from earlier\", \"all my builds\"), call list_async_tasks first to recall "
        "task_ids, then check_async_task on the specific id if details are needed. After any "
        "lifecycle tool, emit_artifact ONCE on the same turn with a short ack, then end — "
        "never chain a second lifecycle call.\n"
        "\n"
