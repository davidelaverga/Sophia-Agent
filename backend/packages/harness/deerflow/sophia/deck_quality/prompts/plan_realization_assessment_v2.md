# Deck Plan-Realization Assessment v2
...
- Sanitized current brief, audience, goal, viewing context, and explicit
  current-request style or brand constraints.
...
2. Convert each explicit plan commitment into a compact claim, then judge only
   ...
   cannot evidence that claim. A sophisticated written plan is not evidence that
   its visual promise survived.
...
4. Score every supplied criterion independently against all observable clauses
   in its 1/3/5 anchors. Mere repetition or literal presence is not strong
   realization. In particular, a recurring rule or rail earns a strong signature
   score only when it is visually distinctive, subject-derived, functional, and
   memorable; generic dividers or alignment lines may realize the plan only
   partially even when they appear on every slide.
5. Compare each promised fingerprint to the actual composition, including its
   relationship, hierarchy, density, and visual function—not just the presence
   of the named boxes, rows, or list items. Treat a generic box flow as literal
   rather than explanatory when it does not reveal the promised mechanism.
6. Distinguish a deliberate explicit style choice from unplanned default-look
   gravity. Never penalize a requested minimal, dark, cream, gradient, table-led,
   text-led, or imagery-light deck merely for following the request. Requested
   style does not prove that the promised signature or subject world is strong.
7. Classify uncertainty as `taste` only when competent reviewers could
   materially disagree about an observable aesthetic realization. Native
   editability, exact font size, contrast ratios, and other mechanical facts are
   outside C; if mentioned at all, classify them as `evidence_limit` with
   `material=false`. They cannot trigger user review or reduce plan-realization
   scores.
...
- `commitments`: every supplied commitment ID exactly once, its dimension,
  `realized`, `partial`, `not_realized`, or `not_applicable`, selectors, and a
  concise observation;
- `criterion_scores`: every supplied criterion ID exactly once, applicability,
  integer score 1–5 when applicable, selectors, and concise anchor-grounded
  rationale;
