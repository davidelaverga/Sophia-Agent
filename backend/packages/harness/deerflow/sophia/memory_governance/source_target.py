...
def build_source_target(*, session: SessionRecord, messages: list[SessionMessageRecord], runs: tuple[ExtractionRun, ...],
                        extractor_model: str, ended_at: str | None = None) -> dict[str, object]:
...
def build_source_target_at_epoch(*, session: SessionRecord, messages: list[SessionMessageRecord], runs: tuple[ExtractionRun, ...],
                                extractor_model: str, snapshot, ended_at: str | None = None) -> dict[str, object]:
...
def _build_source_target(*, session, messages, runs, extractor_model, ended_at=None, snapshot=None):
    ...
    for run in sorted(runs,key=lambda r:str(r.extraction_run_id)):
        ...
        if (selected and all(m.sequence in eligible for m in selected) and run.extractor_contract_version=="mem00.extract.v1" and run.extractor_model==extractor_model
            and run.extractor_prompt_version=="mem0_extraction.md:v1" and hmac.compare_digest(manifest,run.input_manifest_ref)
            and run.source_dependencies==current and context_is_current(run.extractor_input_context,context_mode=session.context_mode)
            and hmac.compare_digest(run.extractor_input_ref,extraction_input_ref(owner_id=session.user_id,session_id=session.session_id,
                messages=_serialize(selected),context=run.extractor_input_context,model=extractor_model))):
            reused.append({"extraction_run_id":str(run.extraction_run_id),"input_manifest_ref":manifest,
                "extractor_input_ref":run.extractor_input_ref,"extractor_input_context":run.extractor_input_context.model_dump(mode="json",by_alias=True),
    ...
    for message in messages:
        if message.sequence in reserved or message.sequence not in eligible:
            if uncovered: break
        else: uncovered.append(message)
    next_range=None
    if uncovered:
        context=capture_context(context_mode=session.context_mode)
        next_range={"sequence_start":uncovered[0].sequence,"sequence_end":uncovered[-1].sequence,
    ...
    payload={"p_user_id":session.user_id,"p_session_id":session.session_id,"p_thread_id":session.thread_id,
        "p_transcript_revision":session.message_revision,"p_target_manifest_ref":_manifest_ref(user_id=session.user_id,session_id=session.session_id,transcript_revision=session.message_revision,messages=messages),
        "p_observed_runs":observed,"p_reused_runs":reused,"p_next_range":next_range,"p_extractor_contract_version":"mem00.extract.v1",
        "p_extractor_model":extractor_model,"p_extractor_prompt_version":"mem0_extraction.md:v1","p_modality":session.mode or "text","p_ended_at":ended_at}
    if snapshot is not None:
