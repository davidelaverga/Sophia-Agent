...
def _file_target_directive_block(target_path: str, task_type: str | None) -> str:
    ...
    if target_ext == "pptx":
        return (
            ...
            "update. The deliverable must come from the ppt-generation image-forward "
            "workflow, not ad hoc python-pptx/write_file loops.\n"
            "\n"
            "HARD rules:\n"
            "  - Read `/mnt/skills/public/ppt-generation/SKILL.md` if needed, "
            "then create one full-slide generated image per slide.\n"
            "  - Generate the hero image first, then one manifest batch for the "
            "remaining slide images with "
            "`/mnt/skills/public/image-generation/scripts/generate.py --slide-visual` "
            "into `/mnt/user-data/outputs/visuals/`; repair only failed images.\n"
            "  - Write `/mnt/user-data/outputs/deck_plan.json` with one `image_path` "
            "per slide plus speaker notes, then run "
            "`python /mnt/skills/public/ppt-generation/scripts/generate.py --plan-file "
            "/mnt/user-data/outputs/deck_plan.json --output-file "
            f"{target_path}`.\n"
