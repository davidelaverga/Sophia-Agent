...
def _file_target_directive_block(target_path: str, task_type: str | None) -> str:
    ...
    if target_ext == "pptx":
        return (
            ...
            "update. The deliverable must come from the HTML-slide presentation "
            "workflow, not ad hoc python-pptx/write_file loops.\n"
            "\n"
            "HARD rules:\n"
            "  - Read `/mnt/skills/public/ppt-generation/SKILL.md` if needed, "
            "then author one self-contained HTML file per slide under "
            "`/mnt/user-data/outputs/slides/`.\n"
            "  - Generate one visual asset per slide with "
            "`/mnt/skills/public/image-generation/scripts/generate.py --slide-visual` "
            "into `/mnt/user-data/outputs/assets/`; do NOT bake slide titles or "
            "bottom narrative into those images because the slide HTML owns that text.\n"
            "  - Call `build_deck_from_slides(output_path=..., title=...)` once to "
            "convert the slide HTML into the requested `.pptx`.\n"
