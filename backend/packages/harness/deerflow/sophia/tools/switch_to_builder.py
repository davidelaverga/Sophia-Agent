...
def switch_to_builder(
    ...
    companion_artifact = state.get("current_artifact") or state.get("previous_artifact") or {}
    user_id = validate_user_id(state.get("user_id", "default_user"))
    ...
    logger.info(
        "[Builder] switch_to_builder queued: task_type=%s tone=%.1f ritual=%s thread_id=%s model=%s",
        task_type,
