...
- **Per-call image-gen timeout.** `image-generation/scripts/generate.py::_openai_client_from_env` now passes `timeout=SOPHIA_IMAGE_GEN_TIMEOUT` (default 600s) + `max_retries=0` (single + batch), killing the ~10-min hang.
...
- **Deck — compile command documented** in `skills/public/ppt-generation/SKILL.md` "Compile Contract": the exact `generate.py --plan-file … --output-file /mnt/user-data/outputs/<slug>.pptx` command, output path is **load-bearing**, and an explicit prohibition on custom `python-pptx` scripts / placeholder names (`t.pptx`).
