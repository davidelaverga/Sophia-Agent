...
- **VQ-5 — PDF enrichment scope**: `_image_generation_enabled` now includes `.pdf ∧ visuals-requested`; per-ext cap (`_image_generation_max_calls`: pdf=2, else 3); `render_markdown_to_pdf` auto-passes `-V coverimage=` when `visuals/cover-*.png` exists (forces titlepage). **(Superseded 2026-06-24: PDFs are now image-gen on by DEFAULT; per-ext IMAGE cap pdf=3, deck=20 — see "Builder visual overhaul (2026-06-24)".)**
...
### Builder visual overhaul (2026-06-24, PR #144)

From the 2026-06-24 JEPA production forensics ([docs/audits/sophia-builder-observability-forensics-2026-06-24.md](../docs/audits/sophia-builder-observability-forensics-2026-06-24.md)): a deck took ~21 min (8 serial gpt-image calls), a report failed terminally on an 11-vs-10 page off-by-one, PDF diagrams repeated one node-link template, and a deck's `.preview.pdf` surfaced as a second card. Fixes, split by deliverable type:

- **Decks — parallel batch image generation, IMAGE cap 20.** `skills/public/image-generation/scripts/generate.py` gained a `--manifest` batch mode (`_run_batch`): N items via `ThreadPoolExecutor` bounded by `SOPHIA_IMAGE_GEN_CONCURRENCY` (default 4), per-item failure isolation, one `IMAGEGEN_BATCH {...}` summary line. The deck guidance (`builder_task.py` `pptx_visual_guidance`) is **hero-anchor**: generate the hero first, then one batch for the rest each referencing the hero. ~16 min serial → ~2–3 min.
- **Image cap counts IMAGES, not invocations.** `_IMAGE_GENERATION_MAX_CALLS=20` (deck), `_IMAGE_GENERATION_MAX_CALLS_PDF=3`. `_image_generation_images_in_command(command, state)` reads `--manifest` item counts (`_manifest_item_count`); `_image_generation_bash_delta` parses the batch summary (`_parse_image_batch_summary`) for generated paths; `_image_generation_block_command` budget-checks by image count. Both the $5/$12 cost ceilings absorb 20×$0.07.
- **PDF — deer-flow original visual path; excalidraw removed.** `generate_excalidraw_diagram` (the fork-custom single-grammar Graphviz tool that produced repetitive node-link figures) was **deleted**; report figures — charts AND structural diagrams (flow/network/mind-map/fishbone/organization-chart/sankey) — route through the upstream `generate_chart` (chart-visualization/scripts/generate.js). `builder_tools.py` no longer inserts it; prompts/skills re-steer to `generate_chart` diagram families. (`graphviz`/`dot` stays installed in `Dockerfile.langgraph` but is now unused.)
- **PDF — conceptual images on by default (cap 3).** `_image_generation_enabled` returns True for `.pdf`/report task types (`_is_pdf_image_generation_target`); image-generation SKILL.md + visual_composition.md steer the ≤3 images to conceptual/editorial figures (no text-in-image, theme-matched) — data/structure always goes through `generate_chart`.
- **PDF page-count gate — tolerance + never-terminal.** `_pdf_page_count_off_target` accepts a ±10% band (`_pdf_page_count_tolerance`, `_PDF_PAGE_COUNT_TOLERANCE_FRACTION`); `_PDF_PAGE_COUNT_REPAIR_MAX=1` with a targeted one-section repair. An off-band-after-repair PDF **ships with `quality_warning="page_count_off_target"` + capped confidence** (`_apply_pdf_page_count_quality_metadata`) — NEVER `artifact_path=null` (restores the "delivered artifact in the requested format is never a fallback" invariant). The old terminal path (`_pdf_page_count_failure_command`/`_fallback`, `_pdf_render_page_count_failed_after_repairs`) is bypassed/unreferenced.
- **Empty/dead figures.** `generate_report_chart._valid_image_response` rejects 0-byte responses; `render_markdown_to_pdf` flags `images_missing`/dead `missing_resources` as a `layout_warning` so the builder spends one bounded repair turn fixing/regenerating the figure before emit.
- **Deck `.preview.pdf` is not a deliverable.** `routers/artifacts.py::_is_builder_support_artifact_path` now lists `.preview.pdf` (matching `artifact_registry._SUPPORT_ARTIFACT_SUFFIXES` + the Supabase leg) so it never surfaces as a second card; the `.pptx` is the sole deliverable, preview bound as the canvas render source.
- **KNOWN FOLLOW-UP (not yet landed): inline artifact card on a failed→restart run.** When a build fails and is restarted, the inline canvas can track the failed run while the success lands under the restart, so the card stays empty while the side panel (thread-wide list) shows it. Gateway snapshot is correct within the 15-min TTL; the fix is frontend run-tracking/reconciliation in `useBuilderCanvas`/`PresenceArtifactPanel` and needs browser E2E verification.

Regression targets: `tests/test_builder_image_generation_cap.py tests/test_image_generation_batch.py tests/test_builder_enrichment_gates.py tests/test_sophia_builder_flow.py tests/test_builder_prompt_contract.py tests/test_render_markdown_to_pdf.py tests/test_generate_report_chart.py tests/test_artifacts_router.py tests/test_sophia_middlewares.py tests/test_sophia_visual_quality_v31.py`.

### Builder report HTML→PDF + deck batch enforcement (2026-06-25, PR #145)

From the 2026-06-25 visual-render-regression forensics ([docs/audits/sophia-builder-visual-render-regression-2026-06-25.md](../docs/audits/sophia-builder-visual-render-regression-2026-06-25.md)): after the 2026-06-24 overhaul routed **all** report figures to `generate_chart`, prod showed every PDF visual broken — the remote Alipay GPT-Vis service rendered data charts as **empty dark canvases** and failed the `flow/network/mind-map/org-chart` diagram families outright. Separately, decks still generated slide images **serially** (the `--manifest` batch was a prompt-only nudge contradicted by the authoritative `ppt-generation/SKILL.md`). Fixes:

- **Reports render via HTML→PDF (inline `<svg>`), not markdown→pandoc or generate_chart.** The model authors ONE self-contained HTML file with inline static `<svg>` charts/diagrams (deterministic, local — no remote service, no client-side JS) plus inlined base print CSS, then calls the new **`render_html_to_pdf`** tool ([sophia/tools/render_html_to_pdf.py](packages/harness/deerflow/sophia/tools/render_html_to_pdf.py) → [sophia/js/render_html_to_pdf.mjs](packages/harness/deerflow/sophia/js/render_html_to_pdf.mjs), headless Chromium via bundled `playwright-core`). It returns the **same JSON result shape** as `render_markdown_to_pdf` (`page_count`/`image_count`/`layout_quality`), so every PDF gate — page-count tolerance, visual-presence via `image_count` (chromium rasterizes inline SVG into PDF `/Image` XObjects), never-terminal downgrade — works unchanged via the shared `_inspect_pdf_layout_with_targets`.
- **Toolset swap, files kept on disk.** `build_builder_tools_for_task_type` (report/document types) now offers `render_html_to_pdf` and NOT `generate_chart`/`render_markdown_to_pdf`. The retired tool files stay on disk (shared page-count-gate helpers + their tests) but are unwired from the builder. `_PDF_CREATION_TOOL_NAMES` gained `render_html_to_pdf` (so result-capture + attempt-detection recognize it); `_forced_pdf_render_tool_choice()` now names `render_html_to_pdf`; the internal attempt helper was renamed `_render_markdown_to_pdf_attempted` → `_pdf_render_attempted`. All model-facing repair/recovery/rejection strings re-steer to HTML+inline-SVG+`render_html_to_pdf`.
- **chart-visualization removed from the builder skills inventory.** Even with the tool gone, leaving the skill inventoried steered the model to `node generate.js` (the broken remote path) via bash — so `_BUILDER_RELEVANT_SKILLS` (builder_task.py) no longer lists it. New skill contract: [skills/public/pdf-report/SKILL.md](../skills/public/pdf-report/SKILL.md) (HTML + a copy-paste inline-SVG pattern library: bar/line/flow/comparison/donut) + [skills/public/pdf-report/assets/report.css](../skills/public/pdf-report/assets/report.css) (base print CSS; render_html_to_pdf supplies page margins + page-number footer). `visual_composition.md` + `builder_obligations.md` updated. The ≤3 conceptual gpt-image allowance stays (referenced as `<img src="visuals/...">` in the HTML; chromium resolves relative refs because the .mjs opens the file via `file://`).
- **Deck hero-anchor batch is harness-enforced.** `ppt-generation/SKILL.md` rewritten to: hero first → ONE `--manifest` batch for the rest (each item referencing the hero). Backstop: `BuilderArtifactMiddleware._deck_batch_directive_rejection` — for a pptx build, after the hero succeeds, a second SINGLE `--slide-visual` call (no `--manifest`) is rejected ONCE with a `[Sophia/deck-batch]` directive (idempotent via `builder_pptx_diagnostics.deck_batch_directive_emitted`; `image_generation_manifest_seen` then allows single-call stray-failure repairs). ~15-20 min serial → ~2-3 min.
- **Per-call image-gen timeout.** `image-generation/scripts/generate.py` passes `timeout=SOPHIA_IMAGE_GEN_TIMEOUT` + `max_retries` to the OpenAI client. **(SUPERSEDED — current defaults: `_image_gen_timeout_seconds()`=120s, `_image_gen_max_retries()`=3, batch concurrency 2; see the 2026-06-27 fix-forward + 2026-06-30 sections. NOTE the worst-case dead-wait on a hung endpoint is the PRODUCT `timeout × (max_retries+1)` ≈ 120×4 ≈ 480-600s, not a single 600s bound — tune `SOPHIA_IMAGE_GEN_TIMEOUT`/`SOPHIA_IMAGE_GEN_MAX_RETRIES`/`SOPHIA_IMAGE_GEN_CONCURRENCY` with that product in mind.)**
- **Deploy:** the langgraph image already installs `chromium` + `npm ci` for `playwright-core` in the js dir (`Dockerfile.langgraph`); `SOPHIA_ARTIFACT_JS_RUNTIME` points the tool at the bundled `.mjs`. Override the browser via `SOPHIA_CHROMIUM_PATH`.

Regression targets: `tests/test_render_html_to_pdf.py tests/test_builder_image_generation_cap.py tests/test_image_generation_batch.py tests/test_image_generation_script.py tests/test_builder_prompt_contract.py tests/test_sophia_builder_flow.py tests/test_visual_quality_regression.py tests/test_sophia_middlewares.py`.

### Builder deck delivery + PDF render fidelity (2026-06-26, PR #146)

From the 2026-06-26 forensics ([docs/audits/sophia-builder-deck-failure-and-pdf-render-forensics-2026-06-26.md](../docs/audits/sophia-builder-deck-failure-and-pdf-render-forensics-2026-06-26.md)): after the HTML→PDF deploy, the PDF path worked but had render imperfections, and **deck builds failed at the compile→emit stage** (image-gen worked — `image_calls=20` fast). Two failure modes, one upstream cause: the `ppt-generation` skill never documented the compile command/output path → the model improvised a custom compile → the `.pptx` landed off-target (`t.pptx`) → the emit gate rejected the model-emitted slug path. Fixes:

- **Deck — `_authoritative_pptx_emit_args` (the core fix; mirrors `_authoritative_pdf_emit_args`).** When `emit_builder_artifact` fires for a `.pptx` target and a valid `.pptx` exists under `/mnt/user-data/outputs/` but the emitted path is missing or a non-pptx fallback, the harness **repoints the emit to the real compiled deck** (via `_preferred_valid_pptx_output_path` → `_promoted_deliverable_from_outputs`) instead of rejecting it — a validly-compiled deck under *any* name (`t.pptx`) is delivered, `artifact_is_fallback=False`. Wired at all three emit decision sites alongside the PDF authoritative call: `_recover_missing_emit_args_if_possible` (after_model) + sync/async `wrap_tool_call`. This subsumes the need for a separate "wrap_tool_call respects the ceiling result" guard — the emit simply succeeds on the first attempt, so the 2-rejection→ceiling→terminal-halt race never triggers.
- **Deck — webhook delivery retry.** `builder_events._post_webhook` now retries transient failures (transport error / 5xx) with `_WEBHOOK_RETRY_BACKOFFS_SECONDS = (2,5,15)` on the daemon thread; 2xx stops, 4xx is unretryable. A single gateway hiccup previously dropped a terminal completion event silently (prod: a deck's ceiling-fallback `status=success` webhook was lost).
- **Deck — compile command documented** in `skills/public/ppt-generation/SKILL.md` (at the time, a `generate.py --plan-file` "Compile Contract"). **(SUPERSEDED 2026-06-26 Spec D Phase 0 — decks are now authored as `slides/*.html` and compiled by the `build_deck_from_slides` tool; there is no `--plan-file` Compile Contract in the current SKILL. See the HTML-slide sections below.)**
- **PDF render fidelity** (`skills/public/pdf-report/assets/report.css`): added a `pre`/`pre code` wrap rule (`white-space: pre-wrap; overflow-wrap: anywhere`) — fixes code lines clipping at the A4 right margin (no horizontal scroll in a PDF); a safe `.cols-2` grid primitive with `min-width: 0` on children (the actual fix for the page-8 table+code collision — grid/flex children default `min-width: auto` and refuse to shrink); and a `.section-label` rule (uppercase + letter-spacing + left padding) mitigating the cover first-glyph clip. `pdf-report/SKILL.md` documents `<pre><code>`, `.cols-2`, `.section-label` + two QA-checklist items. The renderer (`render_html_to_pdf.mjs`) is unchanged — it's a correct thin wrapper; all PDF fixes live in the CSS/skill.
- **Supabase existence-probe robustness:** `download_artifact` and the two `check_artifact*_object_exists` HEAD probes now treat **400 like 404** (benign "no object yet", `debug` log) — Supabase Storage answers 400, not only 404, for the missing-object shape (same family as the 2026-06-12 ledger fix). **Known follow-up (deferred):** the emit-time remote check object-path scheme (`<thread>/<file>`) differs from the durable upload scheme (`artifacts/<user>/<thread>/<artifact_id>/<file>`); `_authoritative_pptx_emit_args` makes the local-disk check pass so the remote leg isn't the blocker, but the scheme should be aligned for the post-restart edge case.

Regression targets: `tests/test_builder_deck_pdf_fixes_2026_06_26.py tests/test_builder_prompt_contract.py tests/test_sophia_middlewares.py tests/test_builder_artifact_ceiling_fallback.py tests/test_render_html_to_pdf.py`.

### Spec D Phase 0 — decks on the HTML render substrate (2026-06-26)

The 2026-06-26 deck fix (above) was the wrong failure mode: re-test showed **two `task_type=presentation` builds (runs 019f0473/019f047a) both hard-ceiling-timed-out** — the model generates 20 images then improvises custom `python-pptx`, **never compiles a `.pptx`** (so `_authoritative_pptx_emit_args` has nothing to repoint). Spec D Phase 0 **deletes the failure class**: decks are now authored as HTML and the harness owns the deterministic conversion (same Chromium substrate as the report path).

- **New deck pipeline:** the model generates each slide's image into `/mnt/user-data/outputs/assets/` (hero + one `--manifest` batch), authors one self-contained 1920×1080 HTML file per slide under `/mnt/user-data/outputs/slides/` (real DOM title/narrative + the image by **relative** `../assets/<file>` path), then calls **`build_deck_from_slides(output_path, title)`** — a deterministic builder tool ([sophia/tools/build_deck_from_slides.py](packages/harness/deerflow/sophia/tools/build_deck_from_slides.py)) that renders each `slides/*.html` to a full-bleed PNG via **`render_html_to_png.mjs`** (new, 1920×1080, `page.screenshot`, same Chromium engine as `render_html_to_pdf.mjs`) and wraps the PNGs into `.pptx` via the existing **`compile_pptx.mjs`** full-bleed compiler. The model never authors/runs deck-compilation code.
- **Toolset:** `build_builder_tools_for_task_type` offers `build_deck_from_slides` (not `render_html_to_pdf`) for presentation task types; reports keep `render_html_to_pdf`. Images are already **local files** (`image-generation/scripts/generate.py::_decode_to_file` base64→disk) — no credentialed downloader is needed (Spec D §2.2 amended); slides reference the local PNG by relative path.
- **Improvisation backstop (§2.5):** `BuilderArtifactMiddleware._deck_improvisation_rejection` (wired in sync+async `wrap_tool_call`) rejects, for `.pptx` targets, any `bash`/code-file tool call carrying deck-compilation signals (`import pptx`/`from pptx`/`pptxgenjs`/`compile_pptx`/`ppt-generation/scripts/generate.py`/`Presentation(`) with a redirect to `build_deck_from_slides`. Slide-HTML (`.html`) authoring is always allowed.
- **Skill/prompt:** `ppt-generation/SKILL.md` rewritten to the HTML-slide contract (slide skeleton + base slide CSS, `assets/`+`slides/` convention, `build_deck_from_slides` call, "never write python-pptx" hard rules); `builder_task.py` `pptx_visual_guidance` + the PPTX completion bullet + the preinstalled-libraries line updated to match. The gpt-image full-slide "zone contract" + `slide_qc.py` OCR path are obsoleted (slide text is now real DOM, checked by the render-and-inspect gate in Phase 1).
- **Deploy:** `Dockerfile.langgraph` verifies `render_html_to_png.mjs` is present (alongside `compile_pptx.mjs`/`render_html_to_pdf.mjs`); reuses bundled `playwright-core` + `SOPHIA_CHROMIUM_PATH`/`SOPHIA_ARTIFACT_JS_RUNTIME`.
...
### Spec D Phase 0 §2.6/§2.7 — deck steering single-source + image-gen hang fix (2026-06-27)

From the 2026-06-27 forensics ([docs/audits/sophia-builder-deck-deploy-gap-forensics-2026-06-27.md](../docs/audits/sophia-builder-deck-deploy-gap-forensics-2026-06-27.md)): two prod presentation builds failed (019f0668 hard-ceiling timeout; 019f0679 silent hang) while PDF/markdown succeeded. Phase 0 added the correct deck path (author `slides/*.html` → `build_deck_from_slides`; harness converts to `.pptx`) but **did not delete the old-flow correction surfaces**, so a struggling deck hit the turn-3 `_pptx_skill_correction_message` commanding the retired `generate.py --plan-file` compiler — which `_deck_improvisation_rejection` then blocked. Deadlock with no path to `build_deck_from_slides` → the model never authored a slide HTML, never compiled a `.pptx`.

- **§2.6 — deck steering is a single source of truth.** All deck-correction injection now routes to ONE gate-agnostic message, `_pptx_compile_latch_message` (build_deck_from_slides HTML flow; never `generate.py`/`--plan-file`/slide-plan-JSON/`image_path`). Deleted the retired functions `_pptx_skill_correction_message`, `_pptx_plan_correction_message`, `_pptx_plan_error_reason`, the dead `_visual_asset_required_message`, and the `_maybe_inject_pptx_plan_correction` injector (`_maybe_inject_pptx_structural_correction` now only does slide-count repair). Swept the sibling old-flow strings: `_visual_design_skill_message` PPTX clause, `_maybe_inject_image_generation_stop`, the slide-count-repair message, `_deck_plan_rejection_message`, the visual-evidence rejection parenthetical, and — caught in adversarial review — the **live** `_visual_presence_rejection_message` PPTX `embed_hint` ("presentation plan JSON / re-run the ppt-generation script") plus the dead-gated `_hero_rejection_message` PPTX wiring. Invariant guard: `test_builder_prompt_contract.py` renders every deck correction (including the gate-locked rejection messages, forced via monkeypatch) and asserts none carry a retired-flow instruction token + the canonical message names `build_deck_from_slides`. **Note:** the on-disk `_deck_plan_validation_problems` "zero embedded pictures" gate is kept (it is inert in the happy path — `build_deck_from_slides` sets `picture_count == slide_count`); only its model-facing message was rewritten.
- **§2.7 — image-gen subprocess hang.** Root cause: prod's `LocalSandboxProvider` ran bash via `subprocess.run(capture_output=True, timeout=600)`; a forked grandchild (the image-gen worker) inherits and holds the stdout pipe, so `subprocess.run`'s post-timeout drain blocks far past the timeout → the builder turn never advances, no completion fires. Fix ([sandbox/local/local_sandbox.py](packages/harness/deerflow/sandbox/local/local_sandbox.py) `_run_command_capture`/`_terminate_process_group`): Unix bash now runs via `Popen(start_new_session=True)` + `communicate(timeout)`; on `TimeoutExpired` the whole process group is SIGKILLed (`os.killpg(proc.pid, SIGKILL)`, falling back to `proc.pid` when the leader already exited) so a forking grandchild can't hold the pipe. Windows keeps `subprocess.run`. Regression: a live forking-grandchild repro asserts the call returns near the timeout (not ~30s). **Operational follow-up (RC-3, not code):** deck #1 yielded 1/8 images — audit `OPENAI_API_KEY` quota on sophia-langgraph; the `c1aa8dc7` missing-asset guard now fails short-yield decks honestly.
- **DEPLOY:** prod was 6 commits behind HEAD (`eabe6058`); the always-injected `builder_obligations.md`/`visual_composition.md` were also stale there. Redeploy `sophia-langgraph` + `sophia-gateway` together — necessary, and now sufficient (the §2.6 residuals are fixed in HEAD).
...
### Deck quality — white space, single .pptx delivery, batch enforcement (2026-06-28)

The first SUCCESSFUL prod deck (run `019f0b8a`, commit `3f4c63b9`: 8/8 images, real `.pptx`) exposed three quality/UX/perf defects, root-caused in [docs/audits/sophia-builder-deck-quality-forensics-2026-06-28.md](../docs/audits/sophia-builder-deck-quality-forensics-2026-06-28.md):

- **WS1 — white-space bands → harness-guaranteed opaque dark backdrop.** `render_html_to_png.mjs` screenshotted with `omitBackground` unset → Chromium painted its default WHITE wherever the model's slide markup left the 1920×1080 frame uncovered (the skeleton darkened only `.slide`, not `html,body`; `.visual img{object-fit:contain}` letterboxed). Fix: the renderer takes a new `--bg-color` (default `#0e1626`) and applies it via CDP `Emulation.setDefaultBackgroundColorOverride` BEFORE the screenshot — JS-independent (page JS is disabled), an opaque `.slide` still paints on top, and uncovered regions render navy not white. `build_deck_from_slides._slide_render_command` plumbs `--bg-color` (`_DECK_BG`). Skeleton hardened too: `html, body { background:#0e1626 }` + an opaque-to-edges hard rule (`ppt-generation/SKILL.md`, `visual_composition.md`).
- **WS2 — a deck delivered BOTH a `.pdf` and a `.pptx` → one .pptx everywhere.** The `.preview.pdf` is a render-only canvas aid (the webapp has no native PPTX renderer), but it leaked as a DOWNLOAD on two surfaces. Root cause was frontend, not backend (`artifact_path` is the `.pptx` on every backend path): `getBuilderArtifactFiles` includes the preview (role `preview`) — needed by the canvas's `resolveCanvasRenderFile` — but `ArtifactsPanel` listed every file (incl. preview) as a download row, and `BuilderCompletionCard` used raw `event.artifact_path`. Fix (frontend only): `ArtifactsPanel` filters `file.role !== 'preview'` from download rows; `BuilderCompletionCard` resolves its download/open href to the `primary` file from `event.artifact_files` (preview can never be selected). The canvas (`ArtifactStage`) is unchanged — it still gets the preview for rendering via the full file list.
- **WS3 — ~17.6-min builder wall-clock → enforce the parallel image batch.** Image-gen ran as ~9 SERIAL single calls (14.9 min) instead of hero + ONE `--manifest` batch; the backstop never fired (prod: 0 `deck_batch_nudge` logs). `_deck_batch_directive_rejection` is hardened: detection broadened to ANY post-hero billable image-gen call without `--manifest` (the prior `--slide-visual`-only matcher missed bare `generate.py` calls); the one-shot escape is removed (keeps rejecting until `image_generation_manifest_seen`), bounded by a safety valve `deck_batch_rejection_count >= _DECK_BATCH_REJECTION_CAP (2)`; a `phase=deck_batch_check` log fires on every image-gen bash (LangSmith run traces are still 403 — see WS-C 2026-06-27 — so prod-log visibility matters). A one-shot **images-before-slides** guard (`_slides_before_images_block_command`, wired in sync+async `wrap_tool_call`) rejects authoring `slides/*.html` before the slide images exist, killing the double-authoring (~8 wasted turns).
- **Operator:** the LangSmith key on `sophia-langgraph` must be **workspace-scoped with `runs:write`** — `LANGSMITH_WORKSPACE_ID` alone makes `/v1/metadata/submit` 204 but `/runs/multipart` still 403, so run traces remain unavailable.

Regression targets: `tests/test_builder_image_generation_cap.py tests/test_build_deck_from_slides.py tests/test_builder_prompt_contract.py tests/test_sophia_middlewares.py` + frontend `BuilderCompletionCard.test.tsx`, `ArtifactsPanel.builder.test.tsx`. **Deploy:** redeploy `sophia-langgraph` + `sophia-gateway` together; the frontend (`BuilderCompletionCard`/`ArtifactsPanel`) ships via Vercel.

### Decks: HTML-slide path RESTORED + partial-image floor + truthful image errors (2026-06-29)

A 12-commit "image-forward" detour (HEAD `23d20a5a`) flipped decks to one full-slide gpt-image per slide (text baked into the bitmap, compiled via `ppt-generation/scripts/generate.py --plan-file deck_plan.json`). Prod produced a **2-of-8-slide deck with clipped titles after a ~26-min loop** (forensics: [docs/audits/sophia-builder-deck-revert-and-trace-forensics-2026-06-29.md](../docs/audits/sophia-builder-deck-revert-and-trace-forensics-2026-06-29.md)). The image-gen outage is **compile-path-agnostic**; the differentiator the detour dropped was the **partial-image floor**. Decks are restored to the **HTML-slide substrate** (model authors `slides/*.html` — real DOM title/narrative + one embedded VISUAL-ONLY image by relative `../assets/<file>` — then calls `build_deck_from_slides`; the harness renders each slide to a full-bleed PNG and wraps to `.pptx`). **This is the authoritative deck contract; the image-forward `generate.py --plan-file` path stays on disk but is UNWIRED.**

- **Wiring** (`builder_tools.py`, `builder_artifact.py`): `build_deck_from_slides` re-added to the presentation toolset; `_pptx_compile_ready` → `_pptx_slide_html_ready` (the load-bearing **floor restore** — the compile latch fires on slide-HTML completeness, decoupled from image yield, so `render_html_to_png`'s placeholder degradation ships a complete deck even with partial images); the image-forward `_slides_before_images_block_command` (which *rejected* `slides/*.html`) deleted; ALL live deck-steering messages flipped to the HTML/`build_deck_from_slides` contract (`_pptx_compile_latch_message`, `_deck_improvisation_rejection`, `_pptx_slide_count_repair_message`, `_visual_design_skill_message`, `_deck_plan_rejection_message`, `_hero_rejection_message`, `_visual_presence_rejection_message`). Inert image-forward machinery left on disk: `_deck_plan_validation_problems` (only flags a compiled deck with slides-but-zero-pictures; `build_deck_from_slides` always sets `picture_count == slide_count`), the `ppt-generation/scripts/generate.py` detection markers, and the compile trace span.
- **Prompts** (`ppt-generation`/`image-generation`/`visual-design` SKILLs, `sophia/{visual_composition,builder_obligations}.md`, `builder_task.py` 3 deck clauses, `start_builder_task._visual_expectations_line`): one unified HTML contract, zero image-forward residue. `--slide-visual` reverts to **VISUAL-ONLY** (title/narrative are real HTML, never baked into the image). Standardized: slide images under `/mnt/user-data/outputs/assets/`, slide HTML under `/mnt/user-data/outputs/slides/`, referenced by relative `../assets/<file>`.
- **Floor + truthful errors** (`builder_artifact.py`, `generate.py` already classifies): `quota_exceeded` added to `_IMAGE_GENERATION_TERMINAL_ERRORS` so a billing/credit outage **fast-fails to the floor** (ship the deck with placeholder visuals + `quality_warning="visuals_partial"`) instead of looping; the `[Sophia/image-generation stop]` message now steers to `build_deck_from_slides` (no more "emit null"); the captured `raw_error` (`IMAGEGEN_FAIL raw_error=…`) is surfaced in the `[BuilderImageGeneration]` log line so the next outage is diagnosable from Render logs even while LangSmith is unavailable. (`rate_limit`/`timeout` stay non-terminal — the SDK retries them and the floor still ships once slide HTML exists.)
- **Contradiction guard** (`test_builder_prompt_contract.py`): the deck-correction invariant test is **flipped + extended** — it now asserts the canonical correction names `build_deck_from_slides` + `slides/` + `../assets/`, and that NO deck correction carries image-forward residue (`_IMAGE_FORWARD_DECK_TOKENS`: `pure image-forward`, `full-slide image/bitmap`, `baked into the bitmap`, `deck_plan.json`, `image_path`, `engine-composed`). `test_ppt_generation_skill_requires_opaque_edges` pins the white-space guarantee (`html, body { … background:#0e1626 }`).
- **KEPT from the detour (not reverted):** the `generate.py` white-pad→**crop** aspect fix (the real white-band cause), the new `generate.py` error classes, the `BuilderCompletionCard` download-filename fix, and manifest pre-validation.
- **Operator:** trace 403 is a wrong-workspace-id, not a key-scope problem — set `LANGSMITH_WORKSPACE_ID=26b7385f-8e69-4a13-b4da-49873ae46191` (EU) on both services; do NOT remove it (the org key needs the tenant).

Regression targets: `tests/test_builder_prompt_contract.py tests/test_build_deck_from_slides.py tests/test_sophia_middlewares.py tests/test_builder_image_generation_cap.py tests/test_start_builder_task.py tests/test_builder_task_visual_preflight.py`. **Deploy:** redeploy `sophia-langgraph` + `sophia-gateway` together.

### Deck restabilization — batch-deadlock escape + slide-quality gate (2026-06-30)

From the 2026-06-30 forensics ([docs/audits/sophia-builder-deck-timeout-and-imperfection-forensics-2026-06-30.md](../docs/audits/sophia-builder-deck-timeout-and-imperfection-forensics-2026-06-30.md), 5-agent verified): two prod presentation builds on deploy-gapped `7d529a00` — an 8-slide deck **timed out** and a 4-slide deck **shipped with imperfections** (garbled baked-in text [fixed by the already-landed `3a242827` visual-only contract], cramped/clipped text, invented template chrome). Spec v4.3 (validated against HEAD `a7e42ce2`). Two P0 code fixes + docs:

- **FIX 1 — image-batch deadlock escape (the 8-slide timeout).** Post-hero, an unreadable `--manifest` is rejected at bash-dispatch by `_unreadable_manifest_rejection` (which is ALSO the pre-spend image-budget read — `_image_generation_images_in_command` → `_manifest_item_count_status`, consumed before bash runs; **do NOT defer it**), and serial fallback calls are rejected by `_deck_batch_directive_rejection`. With no escape the model looped to the 45-turn ceiling (`builder_budget.py:94`). New `_deck_floor_escape_command` (in `_image_generation_block_command`, BEFORE the manifest/batch rejections): once total friction (`manifest_rejection_count` + `deck_batch_rejection_count`, both summing diags) reaches `_DECK_FLOOR_ESCAPE_FRICTION_CAP=2` — or the escape already fired (`deck_floor_escape_emitted`, sticky) — it stops rejecting and emits a `[Sophia/deck-floor]` directive routing the model to author `slides/*.html` + `build_deck_from_slides`, where `render_html_to_png` placeholders any missing visual and the deck ships `visuals_partial`. The manifest rejection now bumps `manifest_rejection_count`. `image-generation/SKILL.md` reinforces "write the manifest in a SEPARATE write_file before the `--manifest` bash call" (the harness reads it at dispatch). Budget-safe: the pre-spend manifest read is untouched.
- **FIX 2 — deterministic slide-quality gate (cramped text + chrome).** New [middlewares/slide_quality.py](packages/harness/deerflow/agents/sophia_agent/middlewares/slide_quality.py) — a `SlideQualityInspector` with a **declarative check list** (`overflow`, `chrome`, `density`) and a designed-in **`GraderConfig` slot that is OFF in v1** ("build the socket, defer the LLM-judge plug"). Wired NOT as a chain middleware (the PNG render is opaque inside the tool) but through `BuilderArtifactMiddleware._deck_builder_result_command` → `_slide_quality_rejection_command`, reusing the visual-embed-gate bounded-repair pattern: ONE re-author (`builder_slide_quality_rejections` < 1 + `_repair_iteration_grantable`), consuming one shared `build_iterations` unit, then soft-pass (advisory, never loop). Counter incremented ONLY in the `wrap_tool_call` rejection Command (hook-order trap). **Overflow signal:** `render_html_to_png.mjs` runs JS-disabled, so it measures content via **CDP `Page.getLayoutMetrics`** (`cssContentSize` vs the 1080 viewport) and emits `overflow=<px>` on stderr; `build_deck_from_slides` parses it (`_overflow_px_from_stderr`, `_SLIDE_OVERFLOW_TOLERANCE_PX=8`) and surfaces `overflow_slides` in its result. **Chrome/density** read the slide HTML source (no render): chrome flags `<nav>`/`<footer>`, navigation roles, `page N of N` footers, and chrome class tokens (eyebrow/navbar/icon-strip/pagination/…); density flags >`_DENSITY_WORD_CAP`(130) visible words or >3 columns. The re-author message reuses existing `assets/` images (never regenerates). `ppt-generation/SKILL.md` gained the chrome ban + density guideline backing the checks.
- **FIX 5 — docs.** Corrected the stale `600s` image-gen default (actual 120s; the dead-wait is the 120s×(retries+1) PRODUCT) and the stale `--plan-file` "Compile Contract" reference (superseded by `build_deck_from_slides`).
- **Already-correct at HEAD (validation-confirmed, NOT re-done):** the visual-only `_SOPHIA_SLIDE_ZONE_CONTRACT` (`3a242827`), the partial-image floor (`build_deck_from_slides.py`), and the PDF figure-variety rule (`pdf-report/SKILL.md` — spec FIX 4 was already implemented). FIX 3 (`not_outputs_path` manifest-path audit) was mis-attributed (Build #1 never ran a batch) and dropped.
- **Operator (Step 0, MANDATORY before this helps):** deploy HEAD to BOTH `sophia-langgraph` + `sophia-gateway`; set `SOPHIA_IMAGE_GEN_TIMEOUT=60`, `SOPHIA_IMAGE_GEN_MAX_RETRIES=1`, `SOPHIA_IMAGE_GEN_CONCURRENCY=4` on langgraph to bound the per-hung-image dead-wait. A redeploy alone fixes neither observed problem at the root — these two fixes do.

Regression targets: `tests/test_slide_quality.py tests/test_builder_image_generation_cap.py tests/test_build_deck_from_slides.py tests/test_sophia_middlewares.py tests/test_builder_prompt_contract.py tests/test_sophia_builder_flow.py`. **Deploy:** redeploy `sophia-langgraph` + `sophia-gateway` together.
...
MEM00 voice retention containment: the shared memory facade denies the four voice caller labels for canonical-governed owners before constructing a provider/reader or touching the legacy cache. This reduced-personalization barrier is not a long-lived-context invalidation protocol. Gateway setup and the shared retrieval contract preserve unavailable receipts even when transport succeeds. Keep other owners unchanged; prove next-input revocation safety before removing the barrier.

MEM00 carried automatic context: `memory_governance/context_state.py` clears retrieval-owned IDs, contents and singular/plural memory blocks. Canonical-governed text/Builder entry works on a clean state copy, returns clearing updates on empty/error/skipped lookup, and replaces rather than merges fresh admissions. The synchronous Builder hook cannot reuse prior memory without retrieval. Non-cohort behavior remains unchanged. This is only entry cleanup: per-model current-governance validation, retained ToolMessages, summaries and enriched task descriptions still require their own fence. Diagnostic certification identity is imported from `memory_governance.identity`, never `flags` (EI124).

Staged retention planner: `memory_governance/retained_context.py` accepts content-free inclusion manifests and complete owner-bound revocation deltas. It never authorizes text. Epoch regression or owner/governance uncertainty fails closed; unbound, intersecting or unprovable contexts rotate; unrelated complete revocations refresh the epoch without rotation. Runtime integration must independently authorize/render current canonical text and actually rebuild state before accepting the next input. The pure planner is not yet called by the runtime; its unit tests do not prove that integration.

MEM00 Builder handoff containment: `allows_unversioned_builder_handoff` denies canonical-cohort and uncertain-configuration inheritance. The outbound resolver receives the trusted resolved owner; `BuilderTaskMiddleware` receives the server-selected owner from the factory, never the delegated parent identity. Governed entry clears `delegation_context.relevant_memories` and replaces previous middleware-owned `<builder_briefing>` blocks. It does not validate or scrub old enriched messages, descriptions, summaries or tool results; per-model retained-context fencing remains required.
