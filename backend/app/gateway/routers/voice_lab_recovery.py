...
def _abort_due_unactivated_provider_from_owner_heartbeat(
    ...
    exact local session, and publish the ordinary completion callback before
    the locator can be consumed.  D02 is deliberately excluded because its
    signed freeze/terminal-proof protocol is stronger and remains mandatory.
    """
    ...
    cleanup_id = getattr(admission, "cleanup_obligation_id", None)
    admission_id = getattr(admission, "admission_id", None)
    provider_session_id = getattr(admission, "resource_id", None)
    if not all(
        isinstance(value, str) and value
        for value in (cleanup_id, admission_id, provider_session_id)
    ):
        return None
    record = sessions._store.find_session_by_cleanup_obligation_id(cleanup_id)
    metadata = getattr(record, "metadata", None)
    synthetic = (
    ...
    if (
        ...
        or synthetic.get("voice_provider_resource_state") != "credential_minted"
        or activated_epoch not in {None, 0}
        or not isinstance(pending_epoch, int)
        or isinstance(pending_epoch, bool)
        or pending_epoch <= 0
        or synthetic.get("voice_provider_activation_receipt") is not None
    ):
        return None
    retention_expires_at = synthetic.get("retention_expires_at")
    provider_expires_at = synthetic.get("provider_expires_at")
    if not isinstance(retention_expires_at, str) or not isinstance(
    ...
    canonical_close, canonical_abort, settlement_sha256 = (
        voice._canonical_browser_provider_settlement(
            provider_session_id,
            [],
            [abort_model],
        )
    ...
    def persist_local(
        ...
        current = sessions._store.get(record.user_id, record.session_id)
    ...
    return close_cleanup_provider_session(
        admission,
        user_id=record.user_id,
        session_id=record.session_id,
        metadata=next_metadata,
        expected_provider_state="credential_minted",
        expected_activated_epoch=None,
        expected_pending_epoch=pending_epoch,
        expected_activation_receipt=None,
    ...
    try:
        ...
        if len(matches) == 1 and body.resource_kind == "provider":
            try:
                ...
                d02_freeze = None
        if (
            body.phase == "heartbeat"
            and d02_freeze is None
            and len(matches) == 1
            and body.resource_kind == "provider"
            and matches[0].status == "credential_minted"
        ):
            try:
                aborted = _abort_due_unactivated_provider_from_owner_heartbeat(
                    matches[0]
                )
            except Exception:  # noqa: BLE001 - retain the locator on ambiguity.
                aborted = None
            if aborted is not None:
                matches = [aborted]
                status = str(getattr(aborted, "status", "unknown"))
        return {
            "authorized": False,
            "status": status,
            "code": (
