...
``sink.register_placeholder(thread_id, chat_id, message_id)`` after
posting the placeholder and before spawning the stream consumer. The
registry is process-local; on gateway restart mid-run, the consumer
loses its placeholder mapping and ``handle()`` falls back to posting a
fresh message (already absorbed by ``_safe_edit``'s except branch on
the channel side).
"""
...
class TelegramWorkBotChatRelaySink:
    ...
    def register_placeholder(self, thread_id: str, chat_id: str, message_id: int) -> None:
        with self._placeholders_lock:
            self._placeholders[thread_id] = (chat_id, message_id)
            self._placeholders.move_to_end(thread_id)
            while len(self._placeholders) > _PLACEHOLDER_LRU_MAX:
                evicted_tid, _ = self._placeholders.popitem(last=False)
                self._last_text.pop(evicted_tid, None)

    def get_placeholder(self, thread_id: str) -> tuple[str, int] | None:
        with self._placeholders_lock:
            return self._placeholders.get(thread_id)
    ...
    async def handle(self, event: BuilderEvent) -> None:
        ...
        placeholder = self.get_placeholder(event.thread_id)
        if not placeholder:
        ...
        if self._last_text.get(event.thread_id) == text:
            return  # no-op edit
        self._last_text[event.thread_id] = text
