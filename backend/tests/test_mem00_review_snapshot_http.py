...
from test_mem00_command_status_pilot import declare_memory_owners  # noqa: F401 - pytest fixture, used by name
...
def test_actual_recap_envelope_is_independent_of_local_file(monkeypatch, declare_memory_owners, case, route):  # noqa: F811 - pytest fixture request
    owner = "review-owner"
    ...
    version = str(uuid4())
    source.read_memory_source_messages.return_value = [
        SessionMessageRecord(message_id="message-1", session_id="review-session", thread_id="review-thread", role="user", content="SYNTHETIC_SOURCE_ONLY", sequence=1, memory_source_version=version)
    ]
    ...
    def transport(request):
        ...
        if case == "outage":
            return httpx.Response(503, json={"detail": "PRIVATE_DATABASE_ERROR"})
        ...
        return httpx.Response(
            200,
            json={
                "schema": "mem00.review.v2",
                "memory_contract_epoch": 1,
                "owner_id": "wrong-owner" if case == "wrong_owner" else owner,
                "session_id": "review-session",
                "thread_id": "review-thread",
                "transcript_revision": 1,
                "source_manifest_ref": body["p_source_manifest_ref"],
                "target_sequence_start": 1,
                "target_sequence_end": 1,
                "finalization": {
                    "kind": "source_target_receipt",
                    "status": "ended",
                ...
                "snapshot_id": "a" * 32,
                "review_filter": "pending_review",
                "extraction_state": state,
                "source_eligibility": {
                    "memory_clear_epoch": 0,
                    "source_snapshot_id": "mem00-source-snapshot-" + "a" * 32,
                    "visible_message_count": 1,
                    "eligible_message_count": 1,
                    "before_clear_count": 0,
                    "accepted_version_changed_count": 0,
                    "acceptance_unproven_count": 0,
                },
                "target_message_count": 1,
                "covered_message_count": 0 if case in {"processing", "retry", "failed", "partial_target"} else 1,
                "run_count": 1,
    ...
    with httpx.Client(transport=httpx.MockTransport(transport)) as http:
        ...
        if expected == 200:
            ...
            if case == "processing":
                assert body["extraction_state"] == "processing"
...
def test_review_cursor_binds_owner_session_snapshot_and_position(monkeypatch):
    monkeypatch.setenv("SOPHIA_MEMORY_REFERENCE_HMAC_SECRET", "r" * 32)
    after = uuid4()
    cursor = encode_review_cursor("owner", "session", "a" * 32, after)
    ...
    for owner, session, value in [("other", "session", cursor), ("owner", "other", cursor), ("owner", "session", cursor + "x"), ("owner", "session", "malformed")]:
        with pytest.raises(MemoryGovernanceConflict):
            decode_review_cursor(owner, session, value)
