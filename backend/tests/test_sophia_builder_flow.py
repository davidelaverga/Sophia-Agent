...
def test_middleware_parity_in_companion_and_builder_chains(monkeypatch, declare_memory_owners):  # noqa: F811 - pytest fixture request
...
def test_presentation_builder_toolset_removes_excalidraw_diagram(monkeypatch, declare_memory_owners) -> None:  # noqa: F811 - pytest fixture request
    declare_memory_owners({"user_123": "legacy"})
...
def test_report_builder_toolset_uses_render_html_to_pdf(monkeypatch, declare_memory_owners) -> None:  # noqa: F811 - pytest fixture request
    declare_memory_owners({"user_123": "legacy"})
    builder_module = importlib.import_module("deerflow.agents.sophia_agent.builder_agent")
    monkeypatch.setenv("LANGSMITH_TRACING", "false")
    _reset_tracing_cache()
...
def test_pdf_presentation_delivery_uses_real_pdf_renderer(monkeypatch, declare_memory_owners) -> None:  # noqa: F811 - pytest fixture request
    declare_memory_owners({"user_123": "legacy"})
    builder_module = importlib.import_module("deerflow.agents.sophia_agent.builder_agent")
    monkeypatch.setenv("LANGSMITH_TRACING", "false")
    _reset_tracing_cache()
...
def test_builder_agent_anthropic_timeout_and_retries(monkeypatch, declare_memory_owners) -> None:  # noqa: F811 - pytest fixture request
    declare_memory_owners({"user_123": "legacy"})
...
def test_builder_factory_returns_langgraph_compatible_graph(monkeypatch, declare_memory_owners) -> None:  # noqa: F811 - pytest fixture request
    declare_memory_owners({"user_123": "legacy"})
...
def test_companion_disables_tracing_on_model_only(monkeypatch, declare_memory_owners):  # noqa: F811 - pytest fixture request
    declare_memory_owners({"user_123": "legacy"})
    companion_module = importlib.import_module("deerflow.agents.sophia_agent.agent")
    captured: dict = {}
