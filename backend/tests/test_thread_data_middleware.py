...
def test_before_agent_uses_configurable_thread_id_when_context_is_none(tmp_path):
    middleware = ThreadDataMiddleware(base_dir=str(tmp_path), lazy_init=True)
    runtime = SimpleNamespace(context=None, config={"configurable": {"thread_id": "thread-from-config"}})
