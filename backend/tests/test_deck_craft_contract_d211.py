...
def test_compact_v2_profile_is_required_in_model_schema_and_bounded() -> None:
    ...
    assert "at least two independent non-nested section or div direct children" in body["description"]
    assert "HTML id unique within its slide" in body["description"]
    assert "same two short anchor IDs may be reused in separate slide fragments" in body["description"]
    assert "shared #id rules scale" in body["description"]
    assert "[a-z][a-z0-9_-]{0,31}" in body["description"]
    assert "lowercase ASCII letter followed by at most 31 lowercase ASCII letters" in body["description"]
    assert "maximum of 32 characters" in body["description"]
    assert "data-deck-id must be unique within its slide" in body["description"]
    assert "data-deck-role must be nonempty" in body["description"]
    assert "data-deck-required must equal true" in body["description"]
    slide_css = schema["$defs"]["DeckSlideInput"]["properties"]["slide_css"]
    assert "omit slide_css or pass an empty string" in slide_css["description"]
    assert "retains its full 1024-byte budget" in slide_css["description"]
    assert "Put all fresh-deck CSS" in slide_css["description"]
