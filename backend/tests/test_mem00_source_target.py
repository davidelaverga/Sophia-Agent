...
def messages(count):
    return [SessionMessageRecord(message_id=f"message-{n}", session_id="session", thread_id="thread", role="user", content=f"SYNTHETIC-{n}", sequence=n, memory_source_version=str(UUID(int=n))) for n in range(1, count + 1)]
...
def run(items, revision=1, **updates):
    ...
    return ExtractionRun(
        ...
        sequence_start=items[0].sequence,
        sequence_end=items[-1].sequence,
        state="succeeded_nonzero",
        ...
        extractor_input_ref=extraction_input_ref(owner_id="owner", session_id="session", messages=_serialize(items), context=context, model=_PIPELINE_MODEL),
        source_dependencies=dependencies(items),
        input_manifest_ref=_manifest_ref(user_id="owner", session_id="session", transcript_revision=revision, messages=items),
...
def test_actual_producer_queues_correction_below_watermark(monkeypatch, declare_memory_owners):  # noqa: F811 - pytest fixture request
    ...
    sessions = MagicMock()
    sessions.get.return_value = source(2)
    sessions.read_memory_source_messages.return_value = current
    sessions.list_messages.side_effect = AssertionError("capped source")
    store = MagicMock()
    store.source_extraction_runs.return_value = history
    store.apply_source_target_at_epoch.return_value = SimpleNamespace(run=None)
    ...
    store.finalize_processed_session.assert_not_called()
    store.finalize_and_enqueue_extraction.assert_not_called()


def test_append_reuses_entire_unchanged_original_input():
    current = messages(12)
    previous = run(current[:10])
...
def test_source_target_new_receipt_domain_does_not_reextract_proven_input():
    ...
    previous = run(current)
    payload = build_source_target(session=source(2), messages=current, runs=(previous,), extractor_model=_PIPELINE_MODEL)
    encoded = json.dumps({key: value for key, value in payload.items() if key not in {"p_idempotency_key", "p_request_digest"}}, sort_keys=True, separators=(",", ":"))
...
def test_historical_batch_without_database_dependencies_is_not_bootstrapped():
    current = messages(10)
    previous = run(current, source_dependencies=None)
...
def test_missing_database_versions_never_becomes_empty_success():
    current = messages(1)
    current[0] = current[0].model_copy(update={"memory_source_version": None})
...
def test_reuse_preserves_original_date_instead_of_recapturing_today():
    ...
    previous = previous.model_copy(update={"extractor_input_context": context, "extractor_input_ref": extraction_input_ref(owner_id="owner", session_id="session", messages=_serialize(current), context=context, model=_PIPELINE_MODEL)})
    payload = build_source_target(session=source(2), messages=current, runs=(previous,), extractor_model=_PIPELINE_MODEL)
    assert payload["p_next_range"] is None
