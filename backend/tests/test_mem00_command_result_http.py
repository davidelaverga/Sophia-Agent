...
def logical_receipt():
    return {"event_id": "20000000-0000-4000-8000-000000000001", "operation_id": "original-operation",
        "event_type": "memory_manual_created", "resulting_lifecycle": "active", "memory_id": MEMORY,
        "candidate_id": None, "content_revision": 1, "memory_governance_revision": 1,
        "user_catalog_generation": 1, "user_revocation_epoch": 0, "idempotent_replay": False}
...
def current_view(state="active", owner="owner"):
    ...
    return {"schema":"mem00.current-memory.v1","owner_id":owner,"memory_id":MEMORY,
...
def test_committed_command_replay_survives_current_state_changes(monkeypatch,declare_memory_owners,action,state):
    ...
    def transport(request):
        ...
        if name==f"sophia_memory_{command_name}":
            ...
            if replay: assert applied[key]==body["p_request_digest"]
            ...
            if action=="permanent-delete":receipt.update(status="accepted_and_fenced",provider_purge="purge_pending",tombstone_id="30000000-0000-4000-8000-000000000001")
            return httpx.Response(200,json=receipt)
        assert name=="sophia_memory_current_view"
        assert body=={"p_user_id":"owner","p_memory_id":MEMORY}
        if state=="outage": return httpx.Response(503,text="PRIVATE_CURRENT_READ_FAILURE")
        value=current_view(state if state in {"active","forgotten","tombstoned","not_found","unavailable"} else "active")
        if state=="wrong_owner": value["owner_id"]="different-owner"
        if state=="tombstoned_text": value["lifecycle"]="tombstoned"
    ...
    with httpx.Client(transport=httpx.MockTransport(transport)) as http:
        ...
        if action=="edit": body.update(expected_content_revision=1,expected_governance_revision=1)
        if action in {"forget","restore","permanent-delete"}:body={"idempotency_key":"original-command-key","expected_governance_revision":1}
...
@pytest.mark.parametrize("fault", ["owner", "target", "receipt_target", "tombstone_text", "revision", "unavailable_text",
    "older_content", "older_governance", "delete_missing_disposition", "delete_revived_text", "delete_missing_fence", "false_erasure"])
def test_command_result_model_rejects_cross_scope_or_noncurrent_text(fault):
    ...
    value = {"schema": "mem00.command-result.v1", "owner_id": "owner", "command_key": "original-command-key",
        "status": "committed", "historical_result_only": True, "receipt": logical_receipt(), "current_view": current_view()}
    if fault == "owner": value["owner_id"] = "wrong-owner"
    if fault == "target": value["current_view"]["memory_id"] = "10000000-0000-4000-8000-000000000002"
    if fault == "receipt_target": value["receipt"]["memory_id"] = None
    if fault == "tombstone_text": value["current_view"]["lifecycle"] = "tombstoned"
    if fault == "revision": value["current_view"]["content_revision"] = 2
    if fault == "unavailable_text": value["current_view"]["status"] = "unavailable"
    if fault == "older_content": value["receipt"]["content_revision"] = 5
    if fault == "older_governance": value["receipt"]["memory_governance_revision"] = 7
    if fault.startswith("delete_") or fault == "false_erasure":
        ...
        value["receipt"].update(event_type="memory_tombstoned", resulting_lifecycle="tombstoned",
            status="accepted_and_fenced", tombstone_id="30000000-0000-4000-8000-000000000001")
        value["privacy_disposition"] = DeletionDisposition().model_dump()
        if fault != "delete_revived_text": value["current_view"] = current_view("tombstoned")
        if fault == "delete_missing_disposition": value["privacy_disposition"] = None
        if fault == "delete_missing_fence": value["receipt"]["tombstone_id"] = None
        if fault == "false_erasure": value["privacy_disposition"]["provider_cleanup"] = "complete"
...
def test_current_read_older_than_original_receipt_keeps_receipt_but_zero_text(revision):
    ...
    result = command_result(owner_id="owner", command_key="original-command-key",
        receipt=CommandReceipt.model_validate(receipt), store=store)
