...
def snapshot_dict(items, epoch=1):
    return {
        "schema": "mem00.source-snapshot.v1",
        "snapshot_id": "mem00-source-snapshot-" + "a" * 32,
        "owner_id": "owner",
        "session_id": "session",
        "thread_id": "thread",
        "transcript_revision": 1,
        "memory_clear_epoch": epoch,
        "context_mode": "life",
        "status": "ended",
        "ended_at": None,
        "sources": [{"message_id": m.message_id, "sequence": m.sequence, "source_version": m.memory_source_version, "acceptance_epoch": epoch, "accepted_version": m.memory_source_version, "eligibility": "eligible"} for m in items],
    }


def plan(items, snapshot, runs=(), **kwargs):
    return build_source_target_at_epoch(session=source(), messages=items, runs=runs, extractor_model=_PIPELINE_MODEL, snapshot=SourceSnapshot.model_validate(snapshot), **kwargs)


def test_new_statements_do_not_share_model_range_with_old_or_late_edited_source():
    items = messages(5)
    snapshot = snapshot_dict(items)
    for index in [0, 1]:
...
@pytest.mark.parametrize("field,value", [("owner_id", "wrong"), ("session_id", "wrong"), ("thread_id", "wrong"), ("transcript_revision", 2), ("status", "active"), ("context_mode", "work"), ("ended_at", "2026-09-09T00:00:00Z")])
...
def test_full_reader_mismatch_is_unavailable_not_harmless_exclusion(change):
    ...
    if change == "omitted":
        ...
        items[0] = items[0].model_copy(update={"memory_source_version": str(UUID(int=99))})
    elif change == "wrong_thread":
        items[0] = items[0].model_copy(update={"thread_id": "wrong"})
    else:
        items[0] = items[0].model_copy(update={"session_id": "wrong"})
...
@pytest.mark.parametrize(
    ...
    [
        ("acceptance_epoch", 2),
        ("acceptance_epoch", True),
        ("acceptance_epoch", "1"),
        ("source_version", "unknown"),
        ("sequence", True),
        ("sequence", 0),
        ("eligibility", "before_clear"),
        ("accepted_version", None),
        ("accepted_version", str(UUID(int=99))),
        ("unexpected_content", "secret"),
    ],
)
def test_partition_rejects_unproven_types_scope_and_extra_fields(field, value):
    snapshot = snapshot_dict(messages(1))
...
def test_snapshot_rejects_malformed_envelopes(change):
    ...
    if change == "duplicate_id":
        snapshot["sources"][1]["message_id"] = snapshot["sources"][0]["message_id"]
    elif change == "duplicate_sequence":
        snapshot["sources"][1]["sequence"] = 1
    elif change == "reordered":
        snapshot["sources"].reverse()
    elif change == "bad_epoch":
        snapshot["memory_clear_epoch"] = True
    elif change == "extra_text":
        snapshot["content"] = "secret"
    else:
        snapshot["ended_at"] = "2026-09-09T00:00:00"
    with pytest.raises(ValidationError):
        SourceSnapshot.model_validate(snapshot)
...
def test_sql_microseconds_match_the_actual_session_reader_millisecond_projection():
    items = messages(1)
    value = snapshot_dict(items)
    value["ended_at"] = "2026-09-09T08:40:00.123456+00:00"
    snapshot = SourceSnapshot.model_validate(value)
...
def test_epoch_and_complete_partition_are_bound_into_original_command_identity():
    items = messages(2)
    first = snapshot_dict(items, epoch=1)
    second = snapshot_dict(items, epoch=2)
    assert plan(items, first)["p_idempotency_key"] != plan(items, second)["p_idempotency_key"]
    changed = deepcopy(first)
    changed["snapshot_id"] = "mem00-source-snapshot-" + "b" * 32
...
def test_reader_uses_fixed_rpc_and_sanitizes_failure():
    ...
    store._rpc = Mock(return_value=snapshot_dict(messages(1)))
...
def target_receipt():
    ...
    snapshot = snapshot_dict(items)
    return {
        "schema": "mem00.source-target-at-epoch.v1",
        ...
        "invalidated_count": 0,
        "source_manifest_ref": "hmac-sha256:transcript-manifest:" + "a" * 64,
        "idempotent_replay": False,
        "historical_result_only": True,
        "memory_clear_epoch": 1,
        "source_snapshot": snapshot,
        "source_target": deepcopy(snapshot),
...
@pytest.mark.parametrize("change", ["numeric_historical", "string_replay", "wrong_epoch", "wrong_run_owner", "unknown_run_epoch", "changed_partition", "wrong_source_manifest", "wrong_schema", "unknown_property", "negative_count"])
def test_target_receipt_rejects_malformed_or_wrong_scope(change):
    value = target_receipt()
    if change == "numeric_historical":
        value["historical_result_only"] = 1
    elif change == "string_replay":
        value["idempotent_replay"] = "true"
    elif change == "wrong_epoch":
        value["memory_clear_epoch"] = 0
    elif change == "wrong_run_owner":
        value["run"]["user_id"] = "other"
    elif change == "unknown_run_epoch":
        value["run"].pop("memory_clear_epoch")
    elif change == "changed_partition":
        value["source_target"]["sources"].pop()
    elif change == "wrong_source_manifest":
        value["source_manifest_ref"] = "not-proof"
    elif change == "wrong_schema":
        value["schema"] = "old"
    elif change == "unknown_property":
        value["content"] = "SYNTHETIC INTERNAL"
    else:
        value["invalidated_count"] = -1
    with pytest.raises(ValidationError):
        EpochSourceTargetReceipt.model_validate(value)


def test_fixed_epoch_rpc_receipt_binding_and_no_fallback():
    value = target_receipt()
    items = messages(1)
    payload = plan(items, snapshot_dict(items))
    payload["p_target_manifest_ref"] = value["source_manifest_ref"]
    store = object.__new__(SupabaseMemoryGovernanceStore)
    store._rpc = Mock(return_value=value)
...
def test_complete_source_occurrences_are_not_display_deduplicated():
    ...
    items = [message.model_copy(update={"content": "SYNTHETIC SAME WORDS", "created_at": "2026-09-09T00:00:00.000Z", "provider_event_id": "same-provider-reference", "turn_id": "same-turn-reference"}) for message in messages(2)]
    assert len(canonical_visible_messages(items)) == 1  # Existing display contract remains unchanged.
    sessions = Mock()
    sessions.read_memory_source_messages.return_value = items
...
def test_snapshot_reader_outage_never_becomes_empty_or_falls_back():
    sessions = Mock()
    sessions.read_memory_source_messages.side_effect = RuntimeError("SYNTHETIC PRIVATE FAILURE")
