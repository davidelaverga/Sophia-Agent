...
def declare_memory_owners(monkeypatch):
    ...
    def declare(owners):
        monkeypatch.setattr(owner_authority, "configured_memory_store", lambda: authority_store(owners["review-owner"], owner="review-owner"))
...
def test_command_lookup_is_owner_scoped_content_free_and_never_reissues(monkeypatch, declare_memory_owners, case):
    ...
    row = {
        "event_id": event_id,
        "operation_id": "synthetic-original-operation",
        ...
        "user_catalog_generation": 7,
        "user_revocation_epoch": 3,
    ...
    def transport(request):
        ...
        if case == "outage":
            return httpx.Response(503, json={"message": "PRIVATE_DATABASE_BODY"})
        value = {"schema": "mem00.command-status.v1", "owner_id": "review-owner", "command_key": "original-stable-command", "status": "committed", "historical_result_only": True, "receipt": row}
        if case == "missing":
            value.update(status="not_found", receipt=None)
        if case == "wrong_owner":
            value["owner_id"] = "other-owner"
        if case == "wrong_key":
            value["command_key"] = "different-stable-command"
        if case == "extra_plaintext":
            row["canonical_content"] = "PRIVATE_OLD_EDIT"
        if case == "tombstoned":
            row.update(event_type="memory_tombstoned", resulting_lifecycle="tombstoned", status="accepted_and_fenced", tombstone_id=str(uuid4()), provider_purge="purge_pending")
