...
import pytest
from test_mem00_c2_builder_source import source  # noqa: F401 - pytest fixture, used by name
...
def test_governed_list_uses_bound_live_status_not_cached_text(monkeypatch, asynchronous, fault):
    ...
    runtime = _runtime({child: {"task_id": child, "thread_id": child, "run_id": run, "agent_name": "sophia_builder", "status": "success", "result": "SYNTHETIC PRIVATE CACHE"}})
    binding = SimpleNamespace(child_run_id=run)

    def resolve(**kwargs):
        assert kwargs == {"child_context_id": child, "child_run_id": run}
        if fault == "binding":
            raise RuntimeError("synthetic binding unavailable")
        return binding
    ...
    def get(**kwargs):
        ...
        if fault == "outage":
            raise RuntimeError("synthetic native outage")
        return {"thread_id": child, "run_id": str(uuid4()) if fault == "wrong-run" else run, "status": "unknown" if fault == "unknown-status" else "running", "result": "MUST NOT RETURN"}

    async def aget(**kwargs):
        return get(**kwargs)

    clients = SimpleNamespace(get_sync=lambda _: SimpleNamespace(runs=SimpleNamespace(get=get)), get_async=lambda _: SimpleNamespace(runs=SimpleNamespace(get=aget)))
...
def test_installed_native_list_closure_and_post_read_fences(monkeypatch, asynchronous, fault):
    ...
    child, run = str(uuid4()), str(uuid4())
    runtime = _runtime({child: {"task_id": child, "thread_id": child, "run_id": run, "agent_name": "sophia_builder", "status": "success", "result": "PRIVATE CACHE"}})
    observed = []
    binding = SimpleNamespace(child_run_id=run)
    ...
    monkeypatch.setattr("deerflow.agents.sophia_agent.middlewares.memory_context.active_governed_tool_guard", lambda: SimpleNamespace(check=check, resolve_child_association=resolve))
    tool = wrapper.make_list_async_tasks_wrapper(native)
    raw = asyncio.run(tool.coroutine(runtime=runtime, status_filter="running")) if asynchronous else tool.func(runtime=runtime, status_filter="running")
    result = json.loads(raw)
...
def test_installed_check_requires_live_binding_and_checkpoint(monkeypatch, asynchronous, fault):
    ...
    def resolve(**kwargs):
        ...
        if admitted and fault == "binding-changed":
            return SimpleNamespace(child_run_id=str(uuid4()))
        return binding

    def check():
        if admitted and fault == "revoked":
            raise RuntimeError("revoked")

    state = {"messages": [], "builder_result": {"status": "completed", "terminal_status": "completed", "artifact_path": "/mnt/user-data/outputs/synthetic-table.md", "summary": "ADMITTED SYNTHETIC RESULT"}}
    ...
    async def aget_run(**kwargs):
        return get_run(**kwargs)

    async def aget_thread(**kwargs):
        return get_thread(**kwargs)

    clients = SimpleNamespace(
        get_sync=lambda _: SimpleNamespace(runs=SimpleNamespace(get=get_run), threads=SimpleNamespace(get=get_thread)), get_async=lambda _: SimpleNamespace(runs=SimpleNamespace(get=aget_run), threads=SimpleNamespace(get=aget_thread))
    )
    monkeypatch.setattr("deerflow.agents.sophia_agent.middlewares.memory_context.active_governed_tool_guard", lambda: SimpleNamespace(check=check, resolve_child_association=resolve, admit_child_checkpoint=admit))
...
def test_check_composes_real_guard_binding_and_checkpoint(source, monkeypatch, fault):  # noqa: F811 - pytest fixture request
    from deepagents.middleware.async_subagents import _build_check_tool
    from langgraph.types import Command

    from deerflow.agents.sophia_agent.middlewares.memory_context import MemoryRunGuard
    from deerflow.sophia.memory_governance.builder_provenance import bind_builder_run, issue_builder_handoff
    ...
    monkeypatch.setattr("deerflow.sophia.memory_governance.owner_authority.resolve_owner_authority", lambda owner, **kwargs: SimpleNamespace(user_id=owner, authority_state="governed"))
    ...
    monkeypatch.setattr(MemoryRunGuard, "_readmit", lambda self, context: RetainedAdmission(ContextTransition("continue", "empty", 0), context, (), uuid4()))
    monkeypatch.setattr(MemoryRunGuard, "_empty_native_surface", lambda self: None)
    guard = MemoryRunGuard(owner_id="owner", scope="life", config={"thread_id": source["thread_id"], "langgraph_auth_user_id": "owner", INPUT_PROOF_KEY: source["input_proof"], INPUT_RUN_KEY: source["run_id"]})
    guard.enter({"messages": source["messages"]})
    monkeypatch.setattr("deerflow.agents.sophia_agent.middlewares.memory_context.active_builder_parent_guard", lambda owner, thread: guard if (owner, thread) == ("owner", source["thread_id"]) else None)
    child, run = str(uuid4()), str(uuid4())
    wire, proof = issue_builder_handoff(guard=guard, owner_id="owner", parent_thread_id=source["thread_id"], child_thread_id=child, source_messages=source["messages"])
    ...
    state = {
        **bound,
        "delegation_context": {"parent_user_id": "owner", "parent_thread_id": str(uuid4()) if fault == "wrong-parent" else source["thread_id"]},
        "builder_result": {"status": "completed", "terminal_status": "completed", "artifact_path": "/mnt/user-data/outputs/synthetic.md", "summary": "SEALED SYNTHETIC RESULT"},
    }
    state[CHECKPOINT_PROOF_KEY] = seal_checkpoint(owner_id="owner", context_id=child, run_id=run, state=state, admission=guard.admission, source_dependencies=guard.source_dependencies)
    if fault == "tampered-result":
        state["builder_result"]["summary"] = "TAMPERED SYNTHETIC RESULT"
    if fault == "source-outage":
        source["store"].unavailable = True

    async def get_run(**kwargs):
        return {"thread_id": child, "run_id": run, "status": "success"}

    async def get_thread(**kwargs):
        return {"thread_id": child, "values": state}

    clients = SimpleNamespace(get_async=lambda _: SimpleNamespace(runs=SimpleNamespace(get=get_run), threads=SimpleNamespace(get=get_thread)))
    monkeypatch.setattr("deerflow.agents.sophia_agent.middlewares.memory_context.active_governed_tool_guard", lambda: guard)
    runtime = _runtime({child: {"task_id": child, "thread_id": child, "run_id": run, "agent_name": "sophia_builder", "status": "success", "result": "PRIVATE CACHE"}})
    tool = wrapper.make_check_async_task_wrapper(_build_check_tool(clients))
