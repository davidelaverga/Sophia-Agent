...
def test_recovery_capability_preserves_signed_candidate_across_gateway_deploy(
    monkeypatch: pytest.MonkeyPatch,
    voice_lab_env: None,
) -> None:
    recovery_secret = "recovery-only-secret-at-least-thirty-two-bytes"
    prior_build = "a" * 40
    ...
    monkeypatch.setenv(
        "SOPHIA_VOICE_LAB_CROSS_DEPLOY_RECOVERY_ENABLED", "true"
    )
    token = _sign(
        _claims(
    ...
    request = voice_router.Request(
        {
            "type": "http",
            "method": "POST",
            "path": "/internal/voice-lab/runs/run-001/recover",
            "headers": [
                (VOICE_LAB_CAPABILITY_HEADER.lower().encode(), token.encode()),
                (
                    VOICE_LAB_RECOVERY_INTERNAL_AUTH_HEADER.lower().encode(),
    ...
    claims = capability_for_voice_lab_recovery(request, "run-001")

    assert claims.expected_deployment["backend"] == prior_build
    assert os.environ["RENDER_GIT_COMMIT"] == BUILD


def test_cross_deploy_recovery_gate_requires_product_kill_switch(
    monkeypatch: pytest.MonkeyPatch,
    voice_lab_env: None,
) -> None:
    recovery_secret = "recovery-only-secret-at-least-thirty-two-bytes"
    ...
    with pytest.raises(HTTPException) as exc_info:
        capability_for_voice_lab_recovery(request, "run-001")
    assert exc_info.value.detail == {
        "code": "voice_lab_capability_deployment_mismatch"
    }
...
def _voice_internal_callback_request() -> voice_router.Request:
    return voice_router.Request(
        {
            "type": "http",
            "method": "POST",
            "path": "/internal/voice-lab/cleanup-admissions/test/authorize",
            "headers": [
                (
...
def test_owner_heartbeat_retains_unactivated_provider_before_deadline(
    monkeypatch: pytest.MonkeyPatch,
    voice_lab_env: None,
) -> None:
    ...
    result = recovery_router.authorize_cleanup_admission_callback(
        admission.admission_id,
        recovery_router.CleanupAdmissionAuthorizeCallback(
            cleanup_obligation_id=admission.cleanup_obligation_id,
            resource_kind="provider",
            resource_id=str(admission.resource_id),
            phase="heartbeat",
        ),
        _voice_internal_callback_request(),
    )

    assert result["authorized"] is False
    assert result["status"] == "credential_minted"
    assert cleanup_fence.cleanup_admissions(
        admission.cleanup_obligation_id
    )[0].status == "credential_minted"
    assert (
        record.metadata["synthetic_voice_lab"]["voice_provider_resource_state"]
        == "credential_minted"
    )


def test_owner_heartbeat_aborts_due_unactivated_ordinary_provider(
    monkeypatch: pytest.MonkeyPatch,
    voice_lab_env: None,
) -> None:
    from app.gateway.routers import voice_lab_recovery as recovery_router
    ...
    assert result["authorized"] is False
    assert result["status"] == "activation_aborted"
    ...
    current = cleanup_fence.cleanup_admissions(admission.cleanup_obligation_id)
    assert len(current) == 1
    assert current[0].status == "activation_aborted"
    settlement_sha256 = cleanup_fence._LOCAL_OBLIGATIONS[
        admission.cleanup_obligation_id
    ]["provider_settlement_sha256"]
    assert isinstance(settlement_sha256, str)
    assert len(settlement_sha256) == 64
    ...
    assert recovery_router._provider_terminal_readback(
        claims,
        record,
...
def test_owner_heartbeat_never_shortcuts_d02_provider_settlement(
    monkeypatch: pytest.MonkeyPatch,
    voice_lab_env: None,
) -> None:
    from app.gateway.routers import voice_lab_recovery as recovery_router
