...
- A hard turn cap in `BuilderArtifactMiddleware` (40 tool-bearing turns) pauses runaway builds with a canonical `partial` result (`continuation_task_id`, `completed_files`, `summary_of_done`). The companion can then call `switch_to_builder` again with `resume_from_task_id=<continuation_task_id>` to continue without redoing work, and `BuilderTaskMiddleware` injects a `<resume_from>` briefing listing the already-completed files.
