-- FC01-R1 / M01-C1: revision-safe transcript snapshot admission.
-- Additive and idempotent. Apply only through the deployment database gate.
...
BEGIN
    ...
    IF current_revision <> p_expected_revision THEN
        -- A stale browser may still have messages the durable snapshot does
        -- not. Preserve those new rows, but never delete or overwrite rows
        -- admitted by the newer revision.
        ...
        RETURN jsonb_build_object(
            'accepted', FALSE,
            'duplicate', upserted_count = 0,
            'conflict', TRUE,
            'previous_revision', current_revision - CASE WHEN upserted_count > 0 THEN 1 ELSE 0 END,
...
REVOKE ALL ON FUNCTION public.sophia_replace_session_messages(TEXT, TEXT, BIGINT, JSONB) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.sophia_replace_session_messages(TEXT, TEXT, BIGINT, JSONB) TO service_role;
