...
const DEFINITIONS: Array<{
  ...
  { name: "speak", title: "Schedule synthetic speech", description: "Inject allowlisted fixture or bounded server-side TTS through the page-owned WebAudio microphone seam. V-P01 follow-up speech requires the unchanged service-minted observation receipt plus current cursor, epoch, and turn preconditions.", scopes: ["voice_lab:run"], annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true, idempotentHint: true }, invoke: (s) => s.speak.bind(s) },
  { name: "wait_for_turn", title: "Wait for one voice observation", description: "Boundedly wait after a monotonic event cursor for one exact transcription, playback, tool, task, UI, session, or operation observation. A satisfied V-P01 assistant-turn wait returns its service-authenticated observation receipt.", scopes: ["voice_lab:read"], annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false, idempotentHint: true }, invoke: (s) => s.waitForTurn.bind(s) },
