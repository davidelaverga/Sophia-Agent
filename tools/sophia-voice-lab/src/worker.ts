...
export class VoiceLabWorker {
  ...
  async #publishRunEvidence(input: {
    ...
    const manifest = {
      ...
      message_revisions: input.intentionallyUnallocated ? { status: "intentionally_unallocated_pre_resource", reason: intentionallyUnavailable!.reason, rows: [] } : projectMessageRevisions(run, eventPage.events),
      utterances: projectUtterances(run, eventPage.events, operations),
      ...
      tool_receipts: projectEvents(productEvents, (event) => event.kind === "tool.call" || event.kind.includes("tool-call") || event.kind.includes("tool_result")),
      builder_receipts: projectEvents(productEvents, (event) => event.kind.includes("builder") || event.kind.includes("task_state") || event.kind.includes("control")),
      durable_projections: { session_finalization: projectEvents(eventPage.events, (event) => event.kind === "session.finalized"), task_cleanup: taskCleanup },
      ui_assertions: projectEvents(productEvents, (event) => event.kind === "ui.projection" || event.kind.includes("artifact")),
      ...
      cleanup_audit: {
        ...
        recovery_receipts: eventPage.events.filter((event) => event.kind === "cleanup.recovery").map((event) => ({ observed_at: event.at.toISOString(), ...event.payload })),
        retention_purge_due_at: run.retentionPurgeDueAt?.toISOString() ?? null,
        retention_purge_pending: run.retentionPurgePending,
