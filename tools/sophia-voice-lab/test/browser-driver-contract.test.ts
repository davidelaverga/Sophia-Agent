...
describe("out-of-band recovery retention contract", () => {
  ...
  it("accepts allocation-free live cleanup before a retention deadline exists", async () => {
    ...
    expect(result.events[0]?.dedupeKey).toMatch(new RegExp(`^recovery:${run.id}:live-complete-retention-unsettled:[a-f0-9]{64}import { describe, expect, it } from "vitest";
...
describe("out-of-band recovery retention contract", () => {
  ...
  it("dedupes exact recovery replays but preserves distinct pending attempt receipts", async () => {
    ...
    expect(second.events[0]?.dedupeKey).toMatch(new RegExp(`^recovery:${run.id}:pending:[a-f0-9]{64}import { describe, expect, it } from "vitest";
