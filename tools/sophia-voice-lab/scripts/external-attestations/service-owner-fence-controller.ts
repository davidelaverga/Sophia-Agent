...
import { ServiceOwnerFenceReceiptSchema, verifyServiceOwnerFence } from "../../src/service-owner-fence.js";
import { genericRecoveryOrigin, readServiceOwnerFencePreflight, RecoveryProductDeploymentSchema } from "./generic-worker-preflight.js";
...
export const ServiceOwnerFenceResumeSchema = z.object({
  prepared, consumed: consumed.optional(), accepted: accepted.optional(), receipt: ServiceOwnerFenceReceiptSchema.optional(),
...
export type ServiceOwnerFenceCheckpoint =
  ...
  | { phase: "receipt"; value: z.infer<typeof ServiceOwnerFenceReceiptSchema> };
...
export async function collectServiceOwnerFence(input: {
  ...
  if (control.genericOwnerDispatch?.consumedAt == null) {
    ...
    const observed = await readServiceOwnerFencePreflight(preflightInput());
  ...
  for (let count = 0; count < Math.ceil(timeout / interval) && now().getTime() <= deadline; count++) {
    ...
    if (!observed || observed.instanceIds.length !== 1 || observed.deploySettledAt === null
      ...
      || sha256(observed.instanceIds[0]!) === sha256(input.allocatedWorkerId)
    ...
    const issuedAt = now();
    const receipt = await signServiceOwnerFenceReceipt({ schema: "sophia.voice-lab.service-owner-fence-receipt.v1", receiptId: input.requestId,
      ...
      providerCleanupProven: false, liveResourcesZeroProven: false, issuedAt: issuedAt.toISOString(), expiresAt: new Date(issuedAt.getTime() + 900000).toISOString(),
