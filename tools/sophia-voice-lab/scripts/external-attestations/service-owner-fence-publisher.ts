...
import { ServiceOwnerFenceReceiptSchema, parseVerifiedServiceOwnerFence, verifyServiceOwnerFence } from "../../src/service-owner-fence.js";
...
export async function publishServiceOwnerFence(input: {
  ...
  const receipt = ServiceOwnerFenceReceiptSchema.parse(input.receipt);
  ...
  if (existing) return { status: "ingested" as const, proofSha256: existing.proofSha256, replay: true, cleanupProven: false as const };
  ...
  try {
    ...
    if (proof) return { status: "ingested" as const, proofSha256: proof.proofSha256, replay: true, cleanupProven: false as const };
