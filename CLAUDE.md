...
Follow-up wave (2026-06-11): generated imagery (gpt-image-2) is **on by default for decks**; the PPTX compositor gained 8 layouts + 4 themes; PDFs render through a themed pandoc template (cover page, footer, auto-TOC, template-less retry on failure). Requires `OPENAI_API_KEY` on sophia-langgraph. Detail: **`backend/CLAUDE.md` → "Image-gen enrichment + artifact-skill quality"**.

Visual overhaul (2026-06-24, PR #144): the image cap now counts **IMAGES, not script invocations** — **20/deck** (generated in ONE parallel batch via `image-generation/scripts/generate.py --manifest`, hero-anchor → concurrent rest) and **3/PDF report** (PDFs now get conceptual/editorial images on by default). The custom single-grammar `generate_excalidraw_diagram` was **removed**; PDF charts AND structural diagrams (flow/network/mind-map/fishbone/org-chart/sankey) all route through the upstream `generate_chart` (chart-visualization) engine. The PDF page-count gate gained a ±10% tolerance band and **never fires a terminal failure** for a rendered PDF (off-band ships with `quality_warning="page_count_off_target"`). Deck `.preview.pdf` is excluded from the deliverable list (no second card). Detail: **`backend/CLAUDE.md` → "Builder visual overhaul (2026-06-24)"**.

Report HTML→PDF + deck batch enforcement (2026-06-25, PR #145): the 2026-06-24 forensics found the remote `generate_chart` (Alipay GPT-Vis) rendered **empty charts** + failed structural diagram families in prod — every PDF visual broken. **Reports now render via HTML→PDF**: the model authors ONE self-contained HTML file with inline static `<svg>` charts/diagrams (deterministic, local, no remote service, no JS) + inlined base print CSS, then calls the new **`render_html_to_pdf`** tool (headless Chromium via bundled `playwright-core`). `generate_chart` AND `render_markdown_to_pdf` are **retired for reports** (files kept on disk for shared page-count-gate helpers + tests); `chart-visualization` is removed from the builder skills inventory. The new result mirrors the markdown-PDF JSON shape, so all PDF gates (page-count tolerance, `image_count` visual-presence, never-terminal) work unchanged. Decks: the hero-anchor `--manifest` batch is now **harness-enforced** (a second serial `--slide-visual` call after the hero is rejected once with a batch directive) and image-gen got a per-call timeout (`SOPHIA_IMAGE_GEN_TIMEOUT`, default 600s, `max_retries=0`). Detail: **`backend/CLAUDE.md` → "Builder report HTML→PDF + deck batch enforcement (2026-06-25)"**.
