...
- **Prior-task memories must never reach the builder** (PR #137) — a "report on Hermes" brief once retrieved a prior "user requested … about OpenClaw" `fact`/`decision` (score ~1.0) and built the wrong subject. Every path that feeds the builder is filtered to `preference` only: `BuilderMem0RetrievalMiddleware` (preference-only category + over-fetch-then-trim + strict `category == "preference"` post-filter, since Mem0 filters after the ranked fetch and treats blank categories as wildcards), and the companion-embedding path (`start_builder_task._resolve_memory_snippets` runs the `task_history` classifier). Write side: `mem0_extraction.md` skip-lists deliverable/build requests and `extraction._candidate_policy_rejection_reason` returns `"task_history"` for them. See `backend/CLAUDE.md` → "Builder memory-contamination guard" for the full contract.
### Luis
