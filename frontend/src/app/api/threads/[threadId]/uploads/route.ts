...
// ``userOwnsThread`` is the shared ownership-lookup helper used by
// both POST and the DELETE proxy under ``[filename]/``; see
// ``./_lib/ownership.ts`` for the full rationale (``/api/v1/``
// prefix, ``/open`` vs ``/list``, fail-closed semantics).
