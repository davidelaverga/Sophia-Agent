...
export async function POST(
  ...
  try {
    ...
    if (!body || typeof body !== 'object' || Array.isArray(body)) {
      return NextResponse.json({ error: 'Invalid forget payload' }, { status: 400 });
    }
    const backendResponse = await fetchSophiaApi(
      `/api/sophia/${encodeURIComponent(userId)}/memories/${encodeURIComponent(memoryId)}/forget`,
    ...
    logger.logError(error, { component: 'api/memories/[memoryId]/forget', action: 'forget_memory', request: req });
