...
export async function POST(
  ...
  { params }: { params: Promise<{ parentThreadId: string; taskId: string; runId: string }> },
) {
  const { parentThreadId, taskId, runId } = await params;
  ...
  const response = await fetchSophiaApi(
    `/api/sophia/${encodeURIComponent(userId)}/threads/${encodeURIComponent(parentThreadId)}/builder-canvas/tasks/${encodeURIComponent(taskId)}/runs/${encodeURIComponent(runId)}/cancel`,
    {
      method: 'POST',
      cache: 'no-store',
      headers: capability
