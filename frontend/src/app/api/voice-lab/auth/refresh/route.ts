...
import {
  ...
  mintGatewayFinalizationCapability,
...
function failure(error: unknown): NextResponse {
  ...
  return NextResponse.json({ ok: false, error: 'voice_lab_refresh_failed' }, { status: 500 });
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const gatewayCapability = mintGatewayFinalizationCapability(grant);
