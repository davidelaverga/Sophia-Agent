...
import { isSessionLeaveGuardSuppressedForInSessionActivity } from './session-annotation-navigation-guard';
...
export function useSessionExitProtection({
  ...
  useEffect(() => {
    const handleBeforeUnload = (event: BeforeUnloadEvent) => {
      ...
      if (isSessionLeaveGuardSuppressedForInSessionActivity()) return;
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      if (isSessionLeaveGuardSuppressedForInSessionActivity()) {
        window.history.pushState({ sophiaResponding: true }, '');
        return;
      }
      openExitConfirm();
