...
- **Codex P1 filter task-history before the companion sees it (post-rebase):** the companion-snippet filter (`start_builder_task._drop_builder_task_history`) ran too late — `Mem0MemoryMiddleware` always retrieves `fact` and injects it into the companion's `<memories>` block *before the model call*, so a stored "user asked for a report about OpenClaw" fact was visible to the model and could be echoed into a new Hermes build's `start_builder_task(description=…)` before any downstream filter ran. Added `_drop_task_history_memories` in `mem0_memory.py` to strip task-history (lexical-only — the per-turn voice path can't afford an LLM) before injection. The injection filter closes the model-sees-it path AND keeps it out of the brief; the snippet filter is now defense-in-depth. **Lesson: filter at the point of first exposure, not just the last hop — a downstream sanitizer is moot once the model has already acted on the tainted input.**
- **Codex P2 classifier in-range + bare-write cue (post-rebase):** two more lexical/classifier gaps. (1) `_classify_task_history_with_llm` set `saw_index` for ANY int before the bounds check, so a misnumbered response like `[5]` for one candidate returned an empty set (read as authoritative "drop nothing") instead of `None` — now only a VALID in-range index counts, so `[5]`/`[5,6]` fall back to lexical. (2) `_DELIVERABLE_CREATION_RE` matched "write up" but not bare "write", so "asked Sophia to write a PDF/document about X" (weak noun) wasn't caught on the lexical/companion path — added bare write/writes/writing/wrote/written (request-verb gate keeps it off the user's own writing).
- **Codex P2 styled one-off build vs standing preference (post-rebase):** `_is_delivery_preference` exempted any style-word snippet with no create/topic cue, so a singular one-off build — "needs **a detailed deck** by Monday", "wants **a concise report** for the board" — was wrongly kept as a preference. Added a one-off guard: a singular article governing a deliverable (`_SINGULAR_DELIVERABLE_RE`) or a deadline (`_DEADLINE_RE`) means it's a one-off build, not a standing preference. Generic/plural style prefs ("reports to be concise") and "their reports concise for the board" still stay.
